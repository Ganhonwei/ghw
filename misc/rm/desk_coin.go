...
func (t *Desk) sendCurrency(userid string, score int64, ltype int32, desc string) {
	...
	if v, ok := t.roles[userid]; ok && v != nil {
		...
		if t.isPrivCashRoom() &&
			(ltype == int32(pb.LOG_TYPE116) || ltype == int32(pb.LOG_TYPE117)) {
			privDiamond = score
...
func (t *Desk) setBetMsg(seat uint32, userid string, num int64, desc string) *pb.RMCoinRaiseInfo {
	...
	switch t.DeskData.Rtype {
	case int32(pb.ROOM_TYPE0): //自由
		t.sendCurrency(userid, (-1 * num), int32(pb.LOG_TYPE116), desc)
	case int32(pb.ROOM_TYPE1): //私人
		switch t.Gmode {
		case 0: // 真金
			t.sendCurrency(userid, (-1 * num), int32(pb.LOG_TYPE116), desc)
		case 1: // 娱乐模式
			t.sendFraction(userid, (-1 * num), int32(pb.LOG_TYPE116))
		}
	}
	// t.broadcast(msg)
	return msg
