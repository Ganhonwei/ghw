...
func (t *Desk) _changeStock(cash_stock, bonus_stock, cash_ming, bonus_ming, cash_an, bonus_an int64) {
	...
	if t.Dtype == int32(pb.DESK_TYPE_NEWBIEW) {
		...
		m := &pb.NewbieStock{
			...
			CashMing:  cash_ming,
			CashAn:    cash_an,
			First:     role.RoundGames[int32(pb.RUMMY)] <= 1,
		}
		t.roomPid.Tell(m)
	}
}
...
func (t *Desk) Lose(seatid uint32, zhahu bool, winner uint32, drop bool, drop_score int) {
	...
	if t.Rtype == int32(pb.ROOM_TYPE1) {
		...
		vb := &event.VBGameTaskEvent{
			GameType: int32(pb.RUMMY),
			Win:      false,
			ActTimes: t.DeskAct.ActTimes, // 回合数
		}
...
func (t *Desk) Win(seatid uint32) {
	...
	t.sendCurrency(userid, score_final, int32(pb.LOG_TYPE117), fmt.Sprintf("rummy%s房间赢分", t.DeskData.Rid)) //加钱
...
func (t *Desk) rummyRecord(seat uint32, num int64) {
	...
	msg := &pb.FreeSetRecord{Gtype: int32(pb.RUMMY), Score: num, Bet: bet}
	if num > 0 { //赢
		msg.Rtype = 1
	} else if num < 0 { //输
		msg.Rtype = -1
	...
	if role.State == data.NoveiceState && role.Round >= uint32(bean.NewbiewToCivil[1]) {
	...
	role.RoundGames[int32(pb.RUMMY)]++
	...
	if err = mq.NatsPublish(mq.TopicGameBets, &pb.PublishGameBets{
		...
		glog.Error("publish user rummy bets error", err)
	}

	// 打码量日志
	log := handler.GameFlowWaterLog(num, int(pb.RUMMY), 0, len(role.RechargeTarge), userid)
...
func (t *Desk) checkGameTime(role *data.DeskRole) bool {
	...
	if role.TotalGameTime >= uint64(bean.NewbiewToCivil[0])*60 && role.Money == 0 {
		return false
	} else {
		return true
	}
