...
func (a *DeskActor) enterDesk(arg *pb.EnterDesk, ctx actor.Context) {
	...
	case int32(pb.ROOM_TYPE2): //百人
		//TODO 优化为后台添加配置
		// gameData := handler.NewFreeGameData(a.Name, "", int32(pb.HUA2))
...
func (a *DeskActor) changeDesk(arg *pb.ChangeDesk, ctx actor.Context) {
	...
	case int32(pb.ROOM_TYPE0): //自由
		...
		gameData := handler.NewCoinGameData(a.Name,
			int32(pb.HUA2), arg.Dtype, arg.Ltype)
		if deskPid, ok := a.spawnDesk(gameData, ctx); ok {
			rsp.Desk = deskPid
			arg.Sender.Tell(rsp)
			return
	...
	case int32(pb.ROOM_TYPE1): //私人
		...
		if deskPid, ok := a.spawnDesk2(gameData, ctx); ok {
			...
			if arg.Gmode == 1 {
				...
				//扣除玩家消耗
				if gameData.Cost > 0 {
					msg2 := handler.ChangeCurrencyMsg((-1 * int64(gameData.Cost)), 0, 0, 0, 0, 0, int32(pb.LOG_TYPE2), arg.Cid, "TP娱乐对战房创建", "")
