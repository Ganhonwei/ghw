...
// '进入房间响应消息
func (t *Desk) freeEnterMsg(userid string) *pb.JHFreeEnterRoomRsp {
	msg := new(pb.JHFreeEnterRoomRsp)
	//房间数据
...
// '获取上庄列表消息
func (t *Desk) dealerListMsg() (msg *pb.JHFreeDealerListRsp) {
	msg = new(pb.JHFreeDealerListRsp)
	if !t.isFree() {
...
func (t *Desk) freeBet(userid string, seatBet uint32,
	...
	t.sendCoin(userid, (-1 * num), int32(pb.LOG_TYPE130))
...
func resFreeBet(seat, beseat uint32, val, coin,
	bets int64, userid string) *pb.JHFreeBetRsp {
	return &pb.JHFreeBetRsp{
		Seat:   seat,
		Beseat: beseat,
		Value:  uint32(val),
		Coin:   coin,
...
// ' 百人输赢趋势
func (t *Desk) freeTrends() *pb.JHFreeTrendRsp {
	msg := new(pb.JHFreeTrendRsp)
...
// ' 百人上局赢家列表
func (t *Desk) freeWiners() *pb.JHFreeWinersRsp {
	msg := new(pb.JHFreeWinersRsp)
...
// ' 百人玩家列表
func (t *Desk) freeRoles() *pb.JHFreeRolesRsp {
	msg := new(pb.JHFreeRolesRsp)
...
// ' 百人坐下
func (t *Desk) freeSit(userid string, arg *pb.JHSitReq) (msg *pb.JHSitRsp) {
...
func (t *Desk) sitCheck(userid string, arg *pb.JHSitReq) pb.ErrCode {
	user := t.getPlayer(userid)
	if user == nil {
		glog.Errorf("userid %s not exist", userid)
		return pb.NotInRoom
	...
	for k, v := range t.DeskFree.Multiple {
		...
		if val != 0 { //表示庄家赢,且大于1倍从玩家身上扣赔付倍数
			for userid, betNum := range tmp {
				...
				//扣除位置数
				t.sendCoin(userid, (-1 * int64(num)), int32(pb.LOG_TYPE131))
...
func (t *Desk) dealerJiesuan1() {
	for k, v := range t.DeskFree.Multiple {
		...
		for userid, betNum := range tmp {
			...
			//扣除位置数
			t.sendCoin(userid, val2, int32(pb.LOG_TYPE131))
...
// ' 发牌消息
func resDraw(seat uint32, state int32, cards []uint32) *pb.JHDrawNtf {
	return &pb.JHDrawNtf{
		Seat:  seat,
		State: state,
		Cards: cards,
...
func resFreeStart(dealer, photo, nickname string, state int32,
	carry int64, dealerNum, left uint32) *pb.JHFreeGamestartNtf {
	return &pb.JHFreeGamestartNtf{
		Dealer:        dealer,
		Photo:         photo,
		State:         state,
		Coin:          carry,
...
// ' 游戏结束消息
func (t *Desk) resOverFree() *pb.JHFreeGameoverNtf {
	var left uint32
	if t.DeskGame.Dealer == "" {
		left = 1
	} else {
