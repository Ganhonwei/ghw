...
func (t *Desk) chatText(arg *pb.ChatTextReq, ctx actor.Context) {
	...
	// 	}
	// 	if vip.EmojiCost > 0 {
	// 		t.sendCurrency(userid, -int64(vip.EmojiCost), int32(pb.LOG_TYPE102), "tp2发表情")
	// 	}
	// }
...
// 触发后的人机本局必会离开桌子（防止穿帮）
// return isCharging
func (t *Desk) chargeInGameRobotClose(userid string) (charging bool) {
	// 匹配
...
// 剧情局plus玩家第一次局内充值后人机第一轮跟注/加注/比牌前会进行1次假充值
func (t *Desk) chargeInGameRobotStoryPlus(userid string) (charging bool) {
	// 匹配
	if t.Rtype != int32(pb.ROOM_TYPE0) {
		return
	}
	...
	if t.model != 3 || !t.model3StoryPlus {
		return
	}
	// 人机是否假充值
	if t.model3StoryPlusRobotCharge {
		return
	}
	...
	if t.DeskAct.ActRechargeTimes[playerSeatid] != 1 {
		return
	}
	t.model3StoryPlusRobotCharge = true

	seatid := role.Seat
	t.robotChargingTime = utils.RandMN(20, 40)
	glog.Infof("robot charging in game mode3plus start: %s, %v", userid, t.robotChargingTime)
	t.chargeInGameBegin(seatid)
	return true
}
...
func (t *Desk) chargeInGameCancel(userid string) {
	...
	seatid := t.getSeatid(userid)
	zlog.Infof("局内充值取消: %s, %d, %#v", userid, t.model, t.chargingAmounts[seatid])
...
func (t *Desk) coinCall(userid string) {
	...
	// 机器人假装充值
	if t.chargeInGameRobotStoryPlus(userid) {
		return
	}
	...
	msg2.Info = t.setBet(seatid, userid, num, fmt.Sprintf("tp2 %s房间跟注", t.DeskData.Rid))
	t.recordBet(num)
...
func (t *Desk) coinRaise(userid string) {
	...
	msg2.Info = t.setBet(seatid, userid, num, fmt.Sprintf("tp2 %s房间加注", t.DeskData.Rid))
	t.recordBet(num)
	msg2.Pot = t.DeskGame.BetNum
	if !t.isSee(seatid) {
		msg2.State |= int32(pb.ACT_BLIND)
...
func (t *Desk) coinReplyBi(userid string, agree bool) {
	if agree { //同意比牌
		...
		msg5 := new(pb.JHCoinReplyBiNtf)
		msg5.Seat = seat
		msg5.Biseat = biseat
		msg5.Agree = true
		msg5.Robot = t.isRobot(msg5.Biseat)
		msg5.Round = int32(t.GetRound())
		t.broadcast(msg5)
		...
		t.pauseGame(PAUSE_REASON_1, 3, seat)
		//延时3秒
		...
		msg.Seat = next                  //发起比牌座位
		msg.Biseat = t.getSeatid(userid) //被比牌座位
		msg.Agree = false
		msg.Robot = t.isRobot(msg.Biseat)
		msg.Round = int32(t.GetRound())
		t.broadcast(msg)
...
func (t *Desk) coinBi(userid string) {
	...
	if t.DeskAct.ActState&int32(pb.ACT_SHOW) == int32(pb.ACT_SHOW) { //show
		...
		msg3.Info = t.setBet(seatid, userid, num, fmt.Sprintf("tp2 %s房间show", t.DeskData.Rid))
		t.recordBet(num)
		...
		// num := t.ActAnte * 2
		msg2 := new(pb.JHCoinCallNtf)
		msg2.Info = t.setBet(seat, userid, num, fmt.Sprintf("tp2 %s房间sideshow", t.DeskData.Rid))
		t.recordBet(num)
		msg2.Pot = t.DeskGame.BetNum
		t.broadcast(msg2)
...
func (t *Desk) getRechargeAmount(seat uint32) (uint32, uint32) {
	...
	amendRoom := table.GetTables().Tp2AmendRoomTable.Get(t.DeskData.Game.Id)

	var chargeAmounts []int32 // 额度档位
	var viceAccountUp int32   // 小号升档
	...
	if conf.StoryGameCharge && t.model == 3 {
		...
		// 剧情库存
		req := &pb.GetTpStoryChargeLoss{GameId: t.DeskData.Game.Id}
		rsp := t.reqRoom(req)
		if response, ok := rsp.(*pb.GotTpStoryChargeLoss); ok {
			chargeAmount.ChargeLoss = response.ChargeLoss
			// 库存最低使用值
			if chargeAmount.ChargeLoss < int64(amendRoom.StoryStockMin) {
				chargeAmount.ChargeLoss = int64(amendRoom.StoryStockMin)
			}
			t1 := target
			if chargeAmount.ChargeLoss < 0 {
				target += int32(-chargeAmount.ChargeLoss)
				chargeAmount.Target = target
			}
			zlog.Infof("剧情模式局内充值: %s, %d -> %d, %#v, %#v", t.getUserid(seat), t1, target, response, conf)

		} else {
			zlog.Infof("剧情模式局内充值2: %s, %d, %#v, %#v", t.getUserid(seat), target, response, conf)
		}
		chargeAmounts = conf.ValueRange
		viceAccountUp = conf.ViceAccountUp

	} else {
		// 非剧情局局内充值
		zlog.Infof("非剧情局局内充值: %s, %d, %#v", t.getUserid(seat), target, amendRoom)
		if amendRoom == nil {
			glog.Errorf("amendRoom charge in game not found: %s", t.DeskData.Game.Id)
			return 0, 0
		}
		chargeAmounts = amendRoom.ChargeInGame
		viceAccountUp = amendRoom.ViceAccountUp
	}
	...
	for i, v := range chargeAmounts {
		if v >= target {
			...
			if player != nil && player.ViceAccount { // 小号强行升档
				...
				zlog.Infof("小号强行升档: %s, %d -> %d, %#v -> %#v, %v", t.getUserid(seat), i, upNext, v, chargeAmounts[upNext], chargeAmounts)
			...
			glog.Debugf("desk=%s user=%s cache charge in game: %#v", t.Rid, t.getUserid(seat), chargeAmount)
...
// 获取局内充值金额配置
func (t *Desk) getRechargeAmountConfig(seatid uint32) tb.Tp2ChargeInGameValueRecord {
	//检查玩家模式
	checkModel := func(model int32, v *tb.Tp2ChargeInGameValueRecord) bool {
		if v.Model == -1 {
			return true
		}
		if model == v.Model {
	...
	//检查玩家阶段
	checkStage := func(stage int32, v *tb.Tp2ChargeInGameValueRecord) bool {
		if v.Stage == -1 {
			return true
		}
		if stage == v.Stage {
	...
	//检查今日充值次数区间
	checkChargeNum := func(num int32, v *tb.Tp2ChargeInGameValueRecord) bool {
	...
	//检查今日充值金额区间
	checkChargeAmount := func(amount int64, v *tb.Tp2ChargeInGameValueRecord) bool {
	...
	//检查今日提现次数区间
	checkWithdrawNum := func(num int32, v *tb.Tp2ChargeInGameValueRecord) bool {
	...
	//检查今日提现金额区间
	checkWithdrawAmount := func(amount int64, v *tb.Tp2ChargeInGameValueRecord) bool {
	...
	//检查盈利区间
	checkWinOrLose := func(amount int64, v *tb.Tp2ChargeInGameValueRecord) bool {
	...
	//检查玩家系数区间
	checkPlayerFactor := func(factor float64, v *tb.Tp2ChargeInGameValueRecord) bool {
	...
	//检查局内充值成功率
	checkChargeInGameSuccessRate := func(rate int32, v *tb.Tp2ChargeInGameValueRecord) bool {
	...
	conf := table.GetTables().Tp2ChargeInGameValueTable.GetDataList()
