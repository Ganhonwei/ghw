...
func (a *Desk) enterDeskCost(userid string) {
	...
	msg2 := handler.ChangeCurrencyMsg(int64(a.DeskData.Cost),
		0, 0, 0, 0, 0, int32(pb.LOG_TYPE2), userid, fmt.Sprintf("TP2加入%s房间", a.DeskData.Rid), "")
...
func (a *Desk) chargeInGameFinish(userid string) {
	...
	a.CheckTpUserModel(seatid)

	a.isCharge = true

	if a.model == 3 {
		a.SetTpUserChargeInGameStory(seatid)
	}
	if amount, ok := a.chargingAmounts[seatid]; ok {
		...
		// 充值>=5000标记为剧情局plus用户
		if a.model == 3 {
			plusAmount := table.GetTables().Tp2StoryPlusTable.Get().ChargeInGameAmount
			if plusAmount >= int32(amount.ChargeAmount) {
				// 标记为剧情plus用户
				a.SetTpUserStoryPlus(seatid)
			}
		}
	}
}
...
func (a *Desk) reqRole(msg interface{}) interface{} {
	timeout := 3 * time.Second
	res1, err1 := a.rolePid.RequestFuture(msg, timeout).Result()
	if err1 != nil {
