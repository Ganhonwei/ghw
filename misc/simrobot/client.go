...
const (
	...
	pongWait       = 120 * time.Second   // Time allowed to read the next pong message from the peer.
	pingPeriod     = (pongWait * 7) / 10 // Send pings to peer with this period. Must be less than pongWait.
...
type Robot struct {
	...
	adjust    bool       //是否调整了货币
	seats     []uint32   //其他玩家座位
	emojiCD   time.Time  //时间cd

	emoji         data.EmojiConfig              //人机表情
...
// 获取一个随机的其他人座位
func (r *Robot) GetRandSeat() uint32 {
	if len(r.seats) > 0 {
...
// 检查表情cd
func (r *Robot) CheckEmojiCD() bool {
	if r.emojiCD.IsZero() {
		r.emojiCD = time.Now()
		return true
	} else {
		now := time.Now()
		duration := now.Sub(r.emojiCD)
...
type user struct {
	...
	//lh and 7up
	LHNowChip uint32 //本轮剩余下注筹码
	LHNowSeat uint32 //本轮位置
}
