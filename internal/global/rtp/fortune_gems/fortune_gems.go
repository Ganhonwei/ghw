...
// 特殊转轴列符号
var SpecialSymbols = []Symbol{ONE, TWO, THREE, FIVE, TEN, FIFTEEN}
var SpecialSymbolsExtra = []Symbol{TWO, THREE, FIVE, TEN, FIFTEEN}
...
func IsValidSymbol(symbol Symbol) bool {
	return symbol >= J && symbol <= FIFTEEN
}
...
func IsInputReelsValid(inputReels map[int32]Symbol, extraBet bool) bool {
	...
	for i := int32(1); i <= 12; i++ {
		...
		if i >= 1 && i <= 9 {
			...
			// 10~12格子只能是ONE~FIFTEEN
			if symbol < ONE || symbol > FIFTEEN {
				return false
			}
			// 如果开启了额外投注，不能有ONE
			if extraBet && symbol == ONE {
				return false
			}
		} else {
			return false
