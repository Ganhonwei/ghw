...
func InitConfig(appid, hamcRSA, key, serverPubRSA, clientPriRSA, notifyUrl, withdrawNotifyUrl, payUrl, payoutUrl string) {
	Config = &BlizzardPayConfig{
		...
		PayNotifyUrl:       fmt.Sprintf("%s%s", notifyUrl, "/blizzardpay"),
		WithDrawNotifyUrl:  fmt.Sprintf("%s%s", withdrawNotifyUrl, "/blizzardpay"),
	...
	service.Regist(service.BLIZZARDPY, Config)
	InitWallet()
}
