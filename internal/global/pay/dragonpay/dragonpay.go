...
var Config *DragonPayConfig

func (t *DragonPayConfig) GetChannelId() uint32 {
	return service.DRAGONPAY
}

func (t *DragonPayConfig) GetChannelName() string {
	return "dragonpay"
}

func (t *DragonPayConfig) CheckMethod(ctx *fasthttp.RequestCtx) bool {
	return ctx.IsPost()
}

func (t *DragonPayConfig) ParseParamMap(ctx *fasthttp.RequestCtx) (map[string]string, error) {
	body := ctx.PostBody()
...
func (t *DragonPayConfig) VerifySign(paramMap map[string]string) (bool, error) {
...
func (t *DragonPayConfig) ParseIncomeParamInfo(paramMap map[string]string) (*service.ParamInfo, error) {
...
func (t *DragonPayConfig) ParseWithdrawParamInfo(paramMap map[string]string) (*service.ParamInfo, error) {
	paramInfo := new(service.ParamInfo)
	paramInfo.OrderId = paramMap["orderId"]
	paramInfo.Status = paramMap["status"]
...
func (t *DragonPayConfig) GetSuccessMsg() string {
	return "success"
}

// Submit 下单
func (t *DragonPayConfig) Submit(order *entity.PayOrder) ([]byte, error) {
...
// 提现下单
func (t *DragonPayConfig) WithdrawSubmit(order *entity.WithdrawOrder) ([]byte, error) {
...
// 组装参数
func (c *DragonPayConfig) compose(order *entity.PayOrder) map[string]string {
...
// 提现组装参数
func (c *DragonPayConfig) WithdrawCompose(order *entity.WithdrawOrder) map[string]string {
	dictionary := make(map[string]string)
...
// 验签
func (c *DragonPayConfig) VerifySing(sign, content string, hash crypto.Hash) bool {
	shaNew := hash.New()
	shaNew.Write([]byte(content))
	hashed := shaNew.Sum(nil)
...
func PaySign(param map[string]string, md5key string) string {
	...
	if md5key != "" {
		result.WriteString(fmt.Sprintf("secret=%s", md5key))
	}
...
func (c *DragonPayConfig) SubmitResponse(body []byte, order *entity.PayOrder) (string, error) {
	result := new(DragonPayOrderRespond)
	...
	if result.Code != 1 { // 没成功TODO
		return "", fmt.Errorf("code:%d,message:%s, OrderNo:%s", result.Code, result.Msg, result.Data.OrderId)
	}
	// if !c.VerifySing(result.Data.Sign, SignContent(param), crypto.SHA256) {
	// 	return "", errors.New("verify sign fail")
	// }
	return result.Data.RechargeUrl, nil
	// return "", nil
}

func (c *DragonPayConfig) ParsePayResult(body []byte) (map[string]string, error) {
...
func (c *DragonPayConfig) ParseFormToMap(ctx *fasthttp.RequestCtx) map[string]string {
	// 创建 map 存储表单数据
...
func (c *DragonPayConfig) ValuesToMap(values url.Values) map[string]string {
...
func (c *DragonPayConfig) WithdrawSubmitResponse(body []byte, order *entity.WithdrawOrder) (bool, string) {
	result := new(DragonPayWithdrawRespond)
	// param := make(map[string]string)
	jsoniter.Unmarshal(body, result)
	// param["code"] = fmt.Sprintf("%d", result.Code)
	// param["message"] = result.Message
...
// 充值查单接口
func (t *DragonPayConfig) InspectSubmit(order *entity.PayOrder) ([]byte, error) {
...
// 充值查单响应
func (c *DragonPayConfig) InspectResponse(body []byte, order *entity.PayOrder) error {
	result := new(DragonPayInspectRespond)
	jsoniter.Unmarshal(body, result)
...
// 充值查单
func (c *DragonPayConfig) InspectCompose(order *entity.PayOrder) map[string]string {
	dictionary := make(map[string]string)
...
// 提现查单接口
func (t *DragonPayConfig) InspectWithdrawSubmit(order *entity.WithdrawOrder) ([]byte, error) {
...
// 提现查单响应
func (c *DragonPayConfig) InspectWdResponse(body []byte, order *entity.WithdrawOrder) int {
	result := new(DragonPayInspectRespond)
	jsoniter.Unmarshal(body, result)
...
// 提现查单
func (c *DragonPayConfig) InspectWdCompose(order *entity.WithdrawOrder) map[string]string {
	dictionary := make(map[string]string)
...
// 余额查询
func (c *DragonPayConfig) BalanceQuery() int64 {
	...
	rsp := new(DragonPayBalanceRespond)
...
func (c *DragonPayConfig) QueryCompose() map[string]string {
