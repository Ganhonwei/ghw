...
func PaySignWithoutKey(param map[string]string, md5key string) string {
	// keys := make([]string, 0, len(param))
	// for k := range param {
	// 	keys = append(keys, k)
	// }
	...
	// var result strings.Builder
	// // 修改这部分，只拼接value，不要key和&
	// for _, v := range keys {
	// 	if val, ok := param[v]; ok && val != "" {
	// 		result.WriteString(val) // 直接写入value
	// 	}
	// }
	...
	return utils.Md5(param["amount"] + param["merchantId"] + param["orderId"] + param["timestamp"] + md5key)
}
