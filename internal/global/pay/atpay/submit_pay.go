...
func (c *PayConfig) SubmitResponse(body []byte, order *entity.PayOrder) (string, error) {
	...
	if result.Code != "0" || result.Data.Status != 0 { // 没成功TODO
		return "", fmt.Errorf("code:%s,message:%s, OrderNo:%s", result.Code, result.Msg, result.Data.TradeNo)
	}
