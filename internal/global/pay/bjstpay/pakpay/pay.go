...
func (c *PayConfig) BalanceQuery() int64 {
	...
	if err != nil {
		glog.Info("[pakpay] BalanceQuery order fail, err,", err)
		return 0
	}
	...
	if rsp.Code != 1000 {
		glog.Info("[pakpay] BalanceQuery order fail, code:", rsp.Code)
		return 0
	}
	glog.Infof("[pakpay] BalanceQuery order success")
