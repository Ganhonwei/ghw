...
func (c *PayConfig) InspectResponse(body []byte, order *entity.PayOrder) error {
	...
	if result.CheckStatus != "success" { // 没成功TODO
		return fmt.Errorf("code:%s", result.CheckStatus)
	}
	if result.ResultStatus != "success" {
		return fmt.Errorf("status:%s", result.ResultStatus)
	}
	return nil
}
...
func (c *PayConfig) InspectCompose(order *entity.PayOrder) map[string]string {
	...
	dictionary["order_no"] = fmt.Sprint(order.OrderID)
...
func (c *PayConfig) InspectWdResponse(body []byte, order *entity.WithdrawOrder) int {
	result := new(PayInspectRespond)
	jsoniter.Unmarshal(body, result)
	if result.CheckStatus != "success" || result.ResultStatus != "success" { // 不成功
		return 3
	}
	if result.ResultStatus == "success" {
		return 2
...
func (c *PayConfig) BalanceQuery() int64 {
	...
	if err != nil {
		glog.Info("[ai2pay] BalanceQuery order fail, err,", err)
		return 0
	}
	...
	if rsp.CheckStatus != "success" {
		glog.Info("[ai2pay] BalanceQuery order fail, err:", rsp.StatusMes)
		return 0
	}
	glog.Infof("[ai2pay] BalanceQuery order success")

	// balance, _ := strconv.ParseFloat(rsp.Currency.Balance, 64)
	return int64(common.YuanToFen(rsp.Currency.Balance))
}
