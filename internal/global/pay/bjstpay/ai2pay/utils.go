...
func PaySign(param map[string]string, md5key string) string {
	...
	if md5key != "" {
		result.WriteString(fmt.Sprintf("%s", md5key))
	}
...
func FmtPhone(phone string) string {
...
func GetBankCode(bank string) (string, error) {
	bankCode, ok := BankCode[bank]
	if ok {
		return bankCode, nil
	}
	return "", errors.New("bank name error")
}
