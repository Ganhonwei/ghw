...
type MjlBlizzardPayConfig struct {
	Merchant           string // 商户号
...
type BlizzardPayOrderRespond struct {
	Code    int    `json:"code"` // 状态码
	Message string `json:"msg"`  // 返回的请求信息
	// Data    JyPayData `json:"data"`
	// Result string          `json:"result"`
	Data BlizzardPayData `json:"data"`
}
...
func InitConfig(appid, hamcRSA, key, serverPubRSA, clientPriRSA, notifyUrl, withdrawNotifyUrl, payUrl, payoutUrl string) {
	Config = &MjlBlizzardPayConfig{
		Merchant:           appid,
		HamcRSA:            hamcRSA,
		Key:                key,
		RSAPubRSA:          serverPubRSA,
		RSAPriRSA:          clientPriRSA,
		PayOrderUrl:        fmt.Sprintf("%s%s", payUrl, "/order/v2/create"),
		WithDrawUrl:        fmt.Sprintf("%s%s", payoutUrl, "/withdraw/apply"),
		PayNotifyUrl:       fmt.Sprintf("%s%s", notifyUrl, "/mjlblizzardpay"),
		WithDrawNotifyUrl:  fmt.Sprintf("%s%s", withdrawNotifyUrl, "/mjlblizzardpay"),
	...
	service.Regist(service.MJLBLIZZARDPAY, Config)
	InitWallet()
}
