...
var Config *MjlBlizzardPayConfig
...
// Submit 下单
func (t *MjlBlizzardPayConfig) Submit(order *entity.PayOrder) ([]byte, error) {
...
// 提现下单
func (t *MjlBlizzardPayConfig) WithdrawSubmit(order *entity.WithdrawOrder) ([]byte, error) {
...
// 组装参数
func (c *MjlBlizzardPayConfig) compose(order *entity.PayOrder) map[string]string {
	dictionary := make(map[string]string)

	// wallets := []string{"BKASH", "NAGAD"}
	...
	dictionary["channelId"] = getwallet()
	// dictionary["bankCode"] = getwallet()
...
// 提现组装参数
func (c *MjlBlizzardPayConfig) WithdrawCompose(order *entity.WithdrawOrder) map[string]string {
	dictionary := make(map[string]string)

	dictionary["appId"] = c.Merchant
	dictionary["outOrderNo"] = order.OrderID
	dictionary["amount"] = fmt.Sprintf("%.2f", float32(order.Amount)/100)
	dictionary["bankName"] = getWallet(order.Bank)
...
// 验签
func (c *MjlBlizzardPayConfig) VerifySing(sign, content string, hash crypto.Hash) bool {
	shaNew := hash.New()
	shaNew.Write([]byte(content))
	hashed := shaNew.Sum(nil)
...
// 验签
func (c *MjlBlizzardPayConfig) VerifyHmacSing(sign, content string, hash crypto.Hash) bool {
...
func (c *MjlBlizzardPayConfig) SubmitResponse(body []byte, order *entity.PayOrder) (string, error) {
...
func (c *MjlBlizzardPayConfig) ParsePayResult(body []byte) (map[string]string, error) {
...
func (c *MjlBlizzardPayConfig) WithdrawSubmitResponse(body []byte, order *entity.WithdrawOrder) (bool, string) {
	result := new(BlizzardPayOrderRespond)
	// param := make(map[string]string)
	err := jsoniter.Unmarshal(body, result)
	if err != nil {
...
// 充值查单接口
func (t *MjlBlizzardPayConfig) InspectSubmit(order *entity.PayOrder) ([]byte, error) {
...
// 充值查单响应
func (c *MjlBlizzardPayConfig) InspectResponse(body []byte, order *entity.PayOrder) error {
	// result := new(BlizzardPayOrderRespond)
	// param := make(map[string]string)
	// err := jsoniter.Unmarshal(body, result)
	// if err != nil {
...
// 充值查单
func (c *MjlBlizzardPayConfig) InspectCompose(order *entity.PayOrder) map[string]string {
	dictionary := make(map[string]string)
...
// 提现查单接口
func (t *MjlBlizzardPayConfig) InspectWithdrawSubmit(order *entity.WithdrawOrder) ([]byte, error) {
	return nil, fmt.Errorf("not implement")
...
// 提现查单响应
func (c *MjlBlizzardPayConfig) InspectWdResponse(body []byte, order *entity.WithdrawOrder) int {
	// result := new(BlizzardPayOrderRespond)
...
// 提现查单
func (c *MjlBlizzardPayConfig) InspectWdCompose(order *entity.WithdrawOrder) map[string]string {
	dictionary := make(map[string]string)
...
// 余额查询
func (c *MjlBlizzardPayConfig) BalanceQuery() int64 {
	return 0
...
func (c *MjlBlizzardPayConfig) QueryCompose() map[string]string {
