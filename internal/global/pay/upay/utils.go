...
func doHttpForm(targetUrl string, body []byte) ([]byte, error) {
	...
	req.Header.Add("Merchant-Code", Config.Merchant)
...
func PaySign(param map[string]any, md5key string) string {
	...
	var result strings.Builder
	for _, v := range keys {
		if val, ok := param[v]; ok && val != nil {
			if valMap, ok := val.(map[string]string); ok {
				result.WriteString(fmt.Sprintf("%s=", v))
				keys2 := make([]string, 0, len(valMap))
				for k := range valMap {
				...
				for _, k := range keys2 {
					if val, ok := param[v]; ok && val != "" {
						// vStr := fmt.Sprintf("%v", valMap[k])
						result.WriteString(fmt.Sprintf("%s=%s&", k, valMap[k]))
					}
				}
				continue
			}
...
func ParsePayResult(body []byte) (map[string]string, error) {
	...
	for k, v := range rsp {
		if f, ok := v.(float64); ok {
			if strings.Contains(k, "stamp") {
				reslut[k] = strconv.FormatFloat(f, 'f', 0, 64)
				continue
			}
			reslut[k] = strconv.FormatFloat(f, 'f', 2, 64)
			continue
		}
		reslut[k] = utils.String(v)
	}
	
	// jsonBytes, _ := jsoniter.Marshal(reslut)
	// glog.Infof("ParsePayResult reslut:%s", string(jsonBytes))
