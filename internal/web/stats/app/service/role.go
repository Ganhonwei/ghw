...
func (this *roleService) loadRoleChannel(role *entity.Role) {
	var roleChannel []entity.RoleChannel
	ListByQ(RoleChannels, bson.M{"role_id": role.Id}, &roleChannel)
	for _, v := range roleChannel {
		perm := entity.ChannelInfo{
			Name: v.ChannelName,
		}
		role.ChannelList = append(role.ChannelList, perm)
	}
}
...
func (this *roleService) SetPerm(roleId string, perms []string) error {
	...
	all := SystemService.GetPermList(true)
...
// 设置渠道查询权限
func (this *roleService) SetRoleChannel(roleId string, channels []string) error {
	role := new(entity.Role)
	Get(Roles, roleId, role)
	if role.Id == "" {
		return errors.New("角色不存在")
	}
	var clist []entity.RoleChannel
	ListByQ(RoleChannels, bson.M{"role_id": roleId}, &clist)
	if len(clist) > 0 {
		// 存在渠道设置,移除之前的
		DeleteAll(RoleChannels, bson.M{"role_id": roleId})
	}
	roleChannel := &entity.RoleChannel{RoleId: roleId}
	for _, v := range channels {
		roleChannel.Id = roleId + "." + v
		roleChannel.ChannelName = v
		Insert(RoleChannels, roleChannel)
	}

	newPerms := make([]string, 0)
	newPerms = append(newPerms, "statistics.datalist")
	newPerms = append(newPerms, "statistics.share")

	m := bson.M{
		"$and": []bson.M{
			{"$or": []bson.M{
				{"perm": "statistics.datalist"},
				{"perm": "statistics.share"},
			}},
			{"role_id": roleId},
		},
	}

	DeleteAll(RolePerms, m)
	rolePerm := &entity.RolePerm{RoleId: roleId}
