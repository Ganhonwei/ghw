...
// 新增、修改支付渠道数据
func (this *payService) AddOrUpdatePayChannelStat(newinfo *entity.PayChannelStatsData) error {
	info := new(entity.PayChannelStatsData)
	GetByQ(PayChannelStatss, bson.M{"date": newinfo.Date, "pay_channel": newinfo.PayChannel}, info)
	if info.Id != "" {
		m := bson.M{"_id": info.Id}
		n := bson.M{
			"total_pay":             newinfo.TotalPay,
			"pay_rate":              newinfo.PayRate,
			"total_pay_amount":      newinfo.TotalPayAmount,
			"total_withdraw":        newinfo.TotalWithdraw,
			"withdraw_rate":         newinfo.WithdrawRate,
			"total_withdraw_amount": newinfo.TotalWithdrawAmount,
			"pay_handling_fee":      newinfo.PayHandlingFee,
			"withdraw_handling_fee": newinfo.WithdrawHandlingFee,
			"yesterday_balance":     newinfo.YesterdayBalance,
			"today_balance":         newinfo.TodayBalance,
			"balance":               newinfo.Balance,
			"pay_channel_balance":   newinfo.PayChannelBalance,
		}
		if Update(PayChannelStatss, m, bson.M{"$set": n}) {
			return nil
		}
		...
		if !Insert(PayChannelStatss, newinfo) {
...
// 记录获取到的第三方余额 ThirdPartyBalances
func (this *payService) AddThirdPartyBalance(newinfo *entity.ThirdPartyBalance) error {
	// 新增
	newinfo.Id = bson.NewObjectId().Hex()
	if !Insert(ThirdPartyBalances, newinfo) {
		return errors.New("写入失败:" + newinfo.Id)
	}
	return nil
}
...
// 修改支付渠道余额
func (this *payService) UpdatePayChannelByBalance(newinfo *entity.PayChannel) error {
	m := bson.M{"_id": newinfo.Id}
	n := bson.M{
		"balance": newinfo.Balance,
	}
	if Update(PayChannels, m, bson.M{"$set": n}) {
		return nil
	}
	return errors.New("更新失败:" + newinfo.Id)
}
