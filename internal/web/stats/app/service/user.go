...
import (
	...
	"goserver/internal/web/stats/app/libs"
	"goserver/pkg/data"
	"goserver/pkg/utils"
...
func GetChargeType(classify entity.ChargeClassify, userMoney int64, userState int) int {
	...
	if classify.None[0] <= money && classify.None[1] > money {
		if userState == data.NoveiceState {
			return data.NewBie
		}
		return data.Civilian
	} else if classify.Normal[0] <= money && classify.Normal[1] > money {
		return data.NormalCharge
	} else if classify.XR[0] <= money && classify.XR[1] > money {
		return data.XR
	} else if classify.ZR[0] <= money && classify.ZR[1] > money {
		return data.ZR
	} else if classify.DR[0] <= money && classify.DR[1] > money {
		return data.DR
	} else {
		return data.CDR
	}
