...
func (this *actionService) UpdateShop(userName string, shop_id string) {
	this.Add("update_shop", userName, "shop_id", shop_id, "")
}
...
func (this *actionService) format(action *entity.Action) {
	...
	case "create_task":
		action.Message = fmt.Sprintf("<b>%s</b> 创建了编号为 <b class='blue'>%s</b> 的发布单。", action.Actor, action.ObjectId)
	case "regist":
