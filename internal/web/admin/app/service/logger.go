...
// 转换为分展示
func (this *loggerService) chipList(list []entity.LogLogin) []entity.LogLogin {
	for k, v := range list {
		c, _ := ConvertToIndiaTime(v.LoginTime.Unix())
		v.LoginTime = c
		c1, _ := ConvertToIndiaTime(v.LogoutTime.Unix())
		v.LogoutTime = c1
		list[k] = v
	}
	return list
...
// 检测账号ip是否存在多个账号登录
func (this *loggerService) CheckAccountByIp(userid string) (bool, error) {
	Isflag := false
	var ips []string
	err := LoginLogs.Find(bson.M{"userid": userid}).Distinct("ip", &ips)
	if err != nil {
		beego.Error("CheckAccount fail err: ", err)
	}
	count1, err := LoginLogs.Find(bson.M{"userid": bson.M{"$ne": userid}, "ip": bson.M{"$in": ips}}).Count()
	if err != nil {
		beego.Error("CheckAccount fail err: ", err)
	}
	if count1 > 0 {
...
// 获取可提现金额日志列表
func (this *loggerService) GetOutDiamondList(page, pageSize int, m bson.M) ([]entity.LogOutDiamond, error) {
...
// 转换为分展示
func (this *loggerService) chipList1(list []entity.LogOutDiamond) []entity.LogOutDiamond {
	for k, v := range list {
...
// 获取可提现金额日志总数
func (this *loggerService) GetOutDiamondTotal(m bson.M) (int64, error) {
	return int64(Count(OutDiamonds, m)), nil
}

// 获取赠送金额日志列表
func (this *loggerService) GetGivediamondList(page, pageSize int, m bson.M) ([]entity.LogOutDiamond, error) {
	var list []entity.LogOutDiamond
	if pageSize == -1 {
		pageSize = 100000
	}
...
// 获取赠送金额日志总数
func (this *loggerService) GetGivediamondTotal(m bson.M) (int64, error) {
	return int64(Count(Givediamonds, m)), nil
}
...
// 根据条件查询IP重复记录表中数据
func (this *loggerService) GetUserIpRecod(m bson.M) ([]entity.UserIpRecords, error) {
	var list []entity.UserIpRecords
	err := IpRecords.Find(m).All(&list)
	return list, err
}

// 根据条件查询IP重复记录表中数据
func (this *loggerService) GetUserIpRecodTotal(m bson.M) (int64, error) {
	return int64(Count(IpRecords, m)), nil
}
...
func (this *loggerService) UserStat1() (n1, n2, n3 int64) {
	//会员总数
	m := bson.M{}
	n1, _ = PlayerService.GetTotal(m)
	//今日新增
	//endTime := utils.Stamp2Time(utils.TimestampToday())
	endTime := TimeToday4()
	m["ctime"] = bson.M{"$gte": endTime}
	n2, _ = PlayerService.GetTotal(m)
	//昨天新增
	//startTime := utils.Stamp2Time(utils.TimestampYesterday())
	startTime := TimeYesterday4()
	m["ctime"] = bson.M{"$gte": startTime, "$lt": endTime}
	n3, _ = PlayerService.GetTotal(m)
	return
}
