...
func init() {
	...
	gmCdnDir = beego.AppConfig.String("gm.cdn.directory")
...
func gmPack(code pb.WebCode, atype pb.ConfigAtype,
	...
	case *pb.ModifyStock:
		...
		msg.Data = result
	case *pb.ModifyTpStoryStock:
		msg2 := b.(*pb.ModifyTpStoryStock)
		result, err2 := msg2.Marshal()
		if err2 != nil {
			return []byte{}, err2
		}
...
func UploadRequest(ImgName string, file multipart.File) error {
	...
	err = writer.WriteField("dir", gmCdnDir)
	if err != nil {
		return err
	}
	// 完成 form-data 数据的写入
...
// 支付节点请求
func PayRequest(code pb.WebCode, method string,
	b interface{}) (*entity.WebResponse, error) {
	body, err1 := json.Marshal(b)
	if err1 != nil {
		return nil, err1
	}
	//request
	targetUrl := payUrl + method
	result, err3 := doHttpPost(targetUrl, body)
	if err3 != nil {
		return nil, err3
	}
...
// 支付节点请求
func PayRequestGet(code pb.WebCode, method string, params url.Values) (*entity.WebResponse, error) {
	//request
	targetUrl := payUrl + method
...
// 支付节点请求
func PayRequestJson(code pb.WebCode, method string, b interface{}, resp interface{}) error {
	body, err1 := json.Marshal(b)
	if err1 != nil {
		return err1
	}
