...
// GetCrashGCYXStat crash高潮涌现
func (s *gameStatsService) GetCrashGCYXStat(page, pageSize int, params map[string]any) (
	...
	sql0 := `
		...
		SUM(CASE WHEN has(crash_strategy_type, 8) THEN 1 ELSE 0 END) strategy_round,
		SUM(CASE WHEN has(crash_strategy_type, 8) THEN bet_amount ELSE 0 END) strategy_bet_sum,
		SUM(CASE WHEN has(crash_strategy_type, 8) THEN score ELSE 0 END) strategy_score_sum,
		SUM(score) score_sum
		FROM game.col_detail FINAL
		WHERE gtype = 7 AND robot = 0 AND win_type IN (1,2,3) %s
		AND userid IN (
			SELECT userid FROM (
				SELECT userid, SUM(bet_amount) bet_total, avg(bet_amount) bet_avg, SUM(score) score_total, max(begin_time) lately_time,
					SUM(CASE WHEN has(crash_strategy_type, 8) THEN 1 ELSE 0 END) lhd_gcyx
				FROM game.col_detail FINAL 
				WHERE gtype = 7 AND robot = 0 AND win_type IN (1,2,3) %s
				%s
				GROUP BY userid
	...
	for _, data := range datas { // 输和赢的统计汇总
		...
		mappingCrashStatGcyxByWinType1(data, stat)
	}
	...
	ck.Select(&users, `
		select userid,nickname,ctime,login_time,money,state,regist_area,
			date_diff('day', ctime, now()) live_days, date_diff('day', login_time, now()) loss_days,crash_gcyx_m
	...
	for _, user := range users {
		...
		crash_gcyx_m := utils.ToInt64(user["crash_gcyx_m"])
	...
	ck.Select(&users_high_avg, `
		...
		FROM (
			SELECT userid, (CASE WHEN has(crash_strategy_type, 8) THEN 0 ELSE 1 END) gcyx
			FROM game.col_detail FINAL
			WHERE gtype = 7 AND robot = 0 AND win_type IN (1,2,3) AND begin_time > 1730419200
				AND userid IN ?
			ORDER BY begin_time
		)
		GROUP BY userid
	...
	for _, data := range summaryDatas {
		mappingCrashStatGcyxByWinType1(data, summary)
	}
	// 汇总后二次计算
	mappingCrashStatGcyx0(summary)
	stats = append([]*entity.LHDGcyxStat{summary}, stats...)
	return
}

func mappingCrashStatGcyxByWinType1(src map[string]any, dst *entity.LHDGcyxStat) {
...
func mappingCrashStatGcyx0(dst *entity.LHDGcyxStat) {
