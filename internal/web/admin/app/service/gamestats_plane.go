...
// GetPlaneGCYXStat 飞机高潮涌现
func (s *gameStatsService) GetPlaneGCYXStat(page, pageSize int, params map[string]any) (
	...
	sql0 := `
		...
		WHERE gtype = 10 AND robot = 0 AND win_type IN (1,2,3) %s
		AND userid IN (
			SELECT userid FROM (
	...
	for _, data := range datas { // 输和赢的统计汇总
		...
		mappingPlaneStatGcyxByWinType1(data, stat)
	}
	...
	ck.Select(&users, `
		select userid,nickname,ctime,login_time,money,state,regist_area,
			date_diff('day', ctime, now()) live_days, date_diff('day', login_time, now()) loss_days,av_gcyx_m
	...
	for _, user := range users {
		...
		av_gcyx_m := utils.ToInt64(user["av_gcyx_m"])
	...
	ck.Select(&users_high_avg, `
		...
		FROM (
			...
			FROM game.col_detail FINAL
			WHERE gtype = 10 AND robot = 0 AND win_type IN (1,2,3) AND begin_time > 1730419200
				AND userid IN ?
			ORDER BY begin_time
		)
		GROUP BY userid
	...
	for _, data := range summaryDatas {
		mappingPlaneStatGcyxByWinType1(data, summary)
	}
	// 汇总后二次计算
	mappingPlaneStatGcyx0(summary)
	stats = append([]*entity.LHDGcyxStat{summary}, stats...)
	return
}

func mappingPlaneStatGcyxByWinType1(src map[string]any, dst *entity.LHDGcyxStat) {
...
func mappingPlaneStatGcyx0(dst *entity.LHDGcyxStat) {
