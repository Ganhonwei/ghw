...
// GetUpdownGcyxStat 7updown高潮涌现
func (s *gameStatsService) GetUpdownGcyxStat(page, pageSize int, params map[string]any) (
	...
	sql0 := `
		...
		SUM(CASE WHEN up_strategy_id = 4 THEN 1 ELSE 0 END) strategy_round,
		SUM(CASE WHEN up_strategy_id = 4 THEN bet_amount ELSE 0 END) strategy_bet_sum,
		SUM(CASE WHEN up_strategy_id = 4 THEN score ELSE 0 END) strategy_score_sum,
		SUM(score) score_sum
		FROM game.col_detail FINAL
		WHERE gtype = 3 AND robot = 0 AND win_type IN (1,2,3) %s
		AND userid IN (
			SELECT userid FROM (
				SELECT userid, SUM(bet_amount) bet_total, avg(bet_amount) bet_avg, SUM(score) score_total, max(begin_time) lately_time,
					SUM(CASE WHEN up_strategy_id = 4 THEN 1 ELSE 0 END) lhd_gcyx
				FROM game.col_detail FINAL 
				WHERE gtype = 3 AND robot = 0 AND win_type IN (1,2,3) %s
				%s
				GROUP BY userid
	...
	for _, data := range datas { // 输和赢的统计汇总
		...
		mapping7updownStatGcyxByWinType1(data, stat)
	}
	...
	ck.Select(&users, `
		select userid,nickname,ctime,login_time,money,state,regist_area,
			date_diff('day', ctime, now()) live_days, date_diff('day', login_time, now()) loss_days,seven_gcyx_m
	...
	for _, user := range users {
		...
		seven_gcyx_m := utils.ToInt64(user["seven_gcyx_m"])
	...
	ck.Select(&users_high_avg, `
		...
		FROM (
			SELECT userid, (CASE WHEN up_strategy_id = 4 THEN 0 ELSE 1 END) gcyx
			FROM game.col_detail FINAL
			WHERE gtype = 3 AND robot = 0 AND win_type IN (1,2,3) AND begin_time > 1730419200
				AND userid IN ?
			ORDER BY begin_time
		)
		GROUP BY userid
	...
	// 汇总后二次计算
	mapping7updownStatGcyx0(summary)
	stats = append([]*entity.LHDGcyxStat{summary}, stats...)
	return
}

func mapping7updownStatGcyxByWinType1(src map[string]any, dst *entity.LHDGcyxStat) {
...
func mapping7updownStatGcyx0(dst *entity.LHDGcyxStat) {
