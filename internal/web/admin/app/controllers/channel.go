...
func (this *ChannelController) List() {
	...
	clist, _ := service.ChannelService.GetChannelClassAll()
	if len(clist) > 0 {
		fmt.Print(clist)
	}
...
func (this *ChannelController) AddChannel() {
	...
	if this.isPost() {
		...
		if err != nil {
			...
			if cid != "" {
				service.ActionService.Add("update_channel", this.auth.GetUser().UserName, "", info.Id, info.Name, "")
			} else {
				service.ActionService.Add("add_channel", this.auth.GetUser().UserName, "", info.Id, info.Name, "")
			}
...
func (this *ChannelController) DelChannel() {
	...
	if err != nil {
		...
		service.ActionService.Add("del_channel", this.auth.GetUser().UserName, "", id, info.Name, "")
	}
}
