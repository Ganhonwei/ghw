...
func (a *DeskActor) enterDesk(arg *pb.EnterDesk, ctx actor.Context) {
		...
		if arg.Dtype == int32(pb.DESK_TYPE_POINTCONTROL) {
			...
			if r, ok := rsp.(*pb.EnteredDesk); ok {
				if r.Error == pb.OK {
					arg.Sender.Tell(r)
					// 新建一张桌子
					gameData := handler.NewFreeGameData(a.Name, "60023", int32(pb.ABAR), int32(pb.DESK_TYPE_POINTCONTROL))
	...
	case int32(pb.ROOM_TYPE2): //百人
		//TODO 优化为后台添加配置
		gameData := handler.NewFreeGameData(a.Name, "60011", int32(pb.ABAR), int32(pb.DESK_TYPE_NORMAL))
		switch arg.Dtype {
		case int32(pb.DESK_TYPE_NEWBIEW):
			gameData = handler.NewFreeGameData(a.Name, "60011", int32(pb.ABAR), int32(pb.DESK_TYPE_NEWBIEW))
		case int32(pb.DESK_TYPE_POINTCONTROL):
			gameData = handler.NewFreeGameData(a.Name, "60023", int32(pb.ABAR), int32(pb.DESK_TYPE_POINTCONTROL))
		case int32(pb.DESK_TYPE_NORMAL_B):
			gameData = handler.NewFreeGameData(a.Name, "60011", int32(pb.ABAR), int32(pb.DESK_TYPE_NORMAL_B))
		}
	...
	case int32(pb.ROOM_TYPE0): //自由
		...
		gameData := handler.NewCoinGameData(a.Name,
			int32(pb.ABAR), arg.Dtype, arg.Ltype)
		if deskPid, ok := a.spawnDesk(gameData, ctx); ok {
			deskPid.Tell(arg)
			return
		}
...
func (a *DeskActor) spawnDesk2(deskData *data.DeskData,
	...
	if deskData.Rtype == int32(pb.ROOM_TYPE1) {
		// 私人房默认为准备状态
		newDesk1.state = int32(pb.STATE_READY)
	}
	//添加新桌子
...
func (a *DeskActor) createDesk(arg *pb.CreateDesk, ctx actor.Context) {
	...
	case int32(pb.ROOM_TYPE1): //私人
		...
		gameData.Game = game
		gameData.Round = uint32(arg.Round) // 房间回合数
		// X秒后未开局强制解散
		delayExpire := config.GetPvpRoom().GameStartWait
		gameData.Expire = utils.Timestamp() + int64(delayExpire)
		gameData.Count = uint32(game.AB.Count) // 人数上限
		gameData.Minimum = game.Minimum        // 最低准入

		if deskPid, ok := a.spawnDesk2(gameData, ctx); ok {
			//创建成功消息
			msg := handler.PackABCreateMsg(gameData)
			arg.Sender.Tell(msg)

			// 娱乐模式创建房间费用
			if arg.Gmode == 1 {
				gameData.Payment = 0 // 房主付房费
				// 获取配置局数
				...
				for i, round := range rounds {
					if round == int(arg.Round) {
						gameData.Cost = uint32(costs[i])
					}
				}
				//扣除玩家消耗
				if gameData.Cost > 0 {
					msg2 := handler.ChangeCurrencyMsg((-1 * int64(gameData.Cost)),
						0, 0, 0, 0, 0, int32(pb.LOG_TYPE2), arg.Cid, "andarbahar创建私人房间", "")
...
func (a *DeskActor) stopNewbiewRoom(ctx actor.Context) {
	idles := make([]*data.DeskBase, 0)
	for _, d := range a.desks {
		if d.Dtype == int32(pb.DESK_TYPE_NEWBIEW) && d.Number <= 0 {
			idles = append(idles, d)
		}
	}
	size := len(idles)
