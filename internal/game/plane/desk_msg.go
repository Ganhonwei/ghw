...
func (a *Desk) ding(ctx actor.Context) {
	...
	if a.robotTimer == 10 {
		a.robotTimer = 0
		robot := table.GetTables().PlaneRobotTable.Get()
			...
			if _, ok := a.CrashBets[k]; ok {
				if _, ok := a.CRASHBack[k]; !ok {
					if utils.RandWan(leavePro1) {
						req := &pb.PLANEBackReq{Userid: k, Pos: 0}
						a.selfPid.Tell(req)
					}
				}
			}
			...
			if _, ok := a.CrashBets1[k]; ok {
				if _, ok := a.CRASHBack1[k]; !ok {
					if utils.RandWan(leavePro2) {
						req := &pb.PLANEBackReq{Userid: k, Pos: 1}
						a.selfPid.Tell(req)
					}
				}
			}
...
func (a *Desk) newbiewDing(ctx actor.Context) {
	...
	// 	if now >= q.BetTime {
	// 		a.selfPid.Tell(&pb.PLANEBetReq{Userid: q.Robotid, Value: uint32(q.Chip)})
	// 	} else {
	// 		removeIndex = i
	// 		break
	// 	}
...
func (a *Desk) handlerStop(ctx actor.Context) {
	...
	for _, pos := range []int32{0, 1} {
		...
		for userid, bet := range crashNextBets {
			...
			a.sendCurrency(userid, bet.Coin, bet.Diamond, int32(pb.LOG_TYPE147), fmt.Sprintf("飞机房间%s位置%d返还下注", a.DeskData.Rid, pos+1))
			// 可提现金加回去
			if bet.Out > 0 {
