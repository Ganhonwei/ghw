...
func (t *Desk) printOver() {
	...
	if t.isFree() {
		...
		glog.Debugf("game over Multiple -> %.2f", float32(t.CRASHMulpitle)/100)
...
func (t *Desk) enterSeat(p *data.DeskRole) {
	...
	count := uint32(table.GetTables().PlaneRoomTable.Get().Count)
	var i uint32
	for i = 1; i <= count; i++ {
...
func (t *Desk) nnLeave(userid string, ctx actor.Context) {
	...
	var err int32 = 0
	if t.state != int32(pb.STATE_OVER) {
		_, bet0 := t.CrashBets[userid]
		_, bet1 := t.CrashBets1[userid]
		_, back0 := t.CRASHBack[userid]
		_, back1 := t.CRASHBack1[userid]
		_, nextBet0 := t.CrashNextBets[userid]
		_, nextBet1 := t.CrashNextBets1[userid]
		// 未下注或已撤离 且下轮未下注
		if (!bet0 || back0) && (!bet1 || back1) && !nextBet0 && !nextBet1 {
			err = int32(pb.LeaveEarly)
		}
	}

	errcode := t.leave(userid, err)
	glog.Debugf("crash Leave userid %s, err %v", userid, errcode)
	if errcode == pb.OK && err != int32(pb.LeaveEarly) {
		// 事件
		t.eventPost(userid, event.Initiative_Leave, &event.InitiativeLeaveEvent{Gtype: int(pb.PLANE)})
		//清除数据
		rsp := new(pb.PLANELeaveRsp)
		rsp.Userid = userid
		// t.updateRank(userid) // 先更新排行榜
...
func (t *Desk) leave(userid string, err int32) pb.ErrCode {
	...
	case int32(pb.ROOM_TYPE2): //百人
		...
		_, nextBet0 := t.CrashNextBets[userid]
		_, nextBet1 := t.CrashNextBets1[userid]
		// 已下注且未撤离且未在结算状态 或下一轮已下注
		if (((bet0 && !back0) || (bet1 && !back1)) && t.state != int32(pb.STATE_OVER)) || (nextBet0 || nextBet1) {
			return pb.GameStartedCannotLeave
		}
	}
...
func (t *Desk) userLeaveDesk(userid string) {
	if v, ok := t.roles[userid]; ok {
		...
		//关掉自动逃离
		t.setAutoLeave(userid, false, 0, 0)
		t.setAutoLeave(userid, false, 0, 1)
		//
		msg := new(pb.LeaveDesk)
		msg.Userid = userid
		msg.Roomid = t.DeskData.Rid
...
func (t *Desk) offlineMsg(userid string) {
	if v, ok := t.roles[userid]; ok && v != nil {
		msg := new(pb.PLANEPushOfflineNtf)
		msg.Userid = userid
