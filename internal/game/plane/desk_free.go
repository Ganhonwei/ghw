...
func (t *Desk) freeInit() {
	...
	// 	if len(games) > 0 {
	// 		glog.Infof("init plane id:%s", t.DeskData.Game.Id)
	...
	t.boomTimer = 0
	//下注时间
	room := table.GetTables().PlaneRoomTable.Get()
...
func (t *Desk) checkNewbiewRobot() {
	robotNum := int(table.GetTables().PlaneRobotTable.Get().Num)
	initScore := table.GetTables().PlaneRobotTable.Get().InitScore
	...
	if more == 0 {
		...
		for i := range more {
			...
			// 自定义头像
			t.getHead(user)
			user.FreeWinMap[int32(pb.PLANE)] = handler.GenNewbiewFreeWin(int32(pb.PLANE))
		...
		for _, k := range kickRobots {
			if u, ok := t.NewbiewRobot[k]; ok {
				...
				//离开消息
				t.broadcast(&pb.PLANELeaveNtf{Userid: k})
			}
		}
	}
}
...
// '进入房间响应消息
func (t *Desk) freeEnterMsg(userid string) *pb.PLANEEnterRoomRsp {
	msg := new(pb.PLANEEnterRoomRsp)
	//房间数据
	// msg.Roominfo = handler._PackCrashFreeRoom(t.DeskData)
	msg.Roominfo = t.packCrashFreeRoom(userid)
	// 下注排行榜
	msg.BetRank = t.packFreeBetRank(10)
...
func (t *Desk) packCrashFreeRoom(userid string) (msg *pb.CRASHFreeRoom) {
	...
	room := table.GetTables().PlaneRoomTable.Get()
	plane0 := table.GetTables().PlaneTable.Get(1)
	plane1 := table.GetTables().PlaneTable.Get(2)
	...
	for i, chip := range plane0.Chips[registArea].Value {
		msg.Chip = append(msg.Chip, uint32(chip))
		if chip == plane0.ChipDefault[registArea] {
			msg.ChipSeat = int32(i)
		}
	}
	for i, chip := range plane1.Chips[registArea].Value {
		msg.Chip1 = append(msg.Chip1, uint32(chip))
		if chip == plane1.ChipDefault[registArea] {
			msg.ChipSeat1 = int32(i)
		}
	}
	msg.AutoCrashDefaultMulpitle0 = int32(plane0.AutoBackDefaultMulpitle[registArea] * 100)
	msg.AutoCrashDefaultMulpitle1 = int32(plane1.AutoBackDefaultMulpitle[registArea] * 100)

	autoLimit0 := plane0.AutoBackMulpitleLimit[registArea]
	...
	autoLimit1 := plane1.AutoBackMulpitleLimit[registArea]
	...
	msg.BetLimit0 = plane0.BetLimit[registArea].Value
	msg.BetLimit1 = plane1.BetLimit[registArea].Value
	msg.BetDefault0 = int64(plane0.BetDefault[registArea])
	msg.BetDefault1 = int64(plane1.BetDefault[registArea])

	msg.WinLimit0 = plane0.WinLimit[registArea]
	msg.WinLimit1 = plane1.WinLimit[registArea]
	return
}
...
func (t *Desk) freeSeatRoleMsg(userid string) (msg *pb.CRASHFreeUser) {
	...
	msg = handler.PackPlaneFreeUser(user)
	if t.DeskFree == nil {
		return
	}
	// 赢分
	if w, ok := user.FreeWinMap[int32(pb.PLANE)]; ok {
		for _, v2 := range w {
			if v2.Score > 0 {
				msg.Wins += v2.Score
			}
...
func (t *Desk) freeSeatBetsMsg() (msg []*pb.CRASHFreeUser) {
	...
	for k, u := range t.NewbiewRobot {
		usermsg := handler.PackPlaneFreeUser(u)
		if t.DeskFree == nil {
			continue
		}
		// 胜场
		if w, ok := u.FreeWinMap[int32(pb.PLANE)]; ok {
			for _, v2 := range w {
				// if v2.Wtype == 1 {
				// }
...
// 打包下注排行榜
func (t *Desk) packFreeBetRank(size int) []*pb.CRASHBetRankUser {
	len := min(size, len(t.CRASHDeskFree.BetRank))
...
// 百人取消下注
func (t *Desk) freeCancelBet(userid string, nextBet bool, pos int32) (rsp *pb.PLANECancelBetRsp) {
	...
	t.sendCurrency(userid, bet.Coin, bet.Diamond, int32(pb.LOG_TYPE147), fmt.Sprintf("飞机房间%s位置%d取消下注", t.DeskData.Rid, pos+1))
...
func (t *Desk) freeBet(userid string, num int64, pos int32) pb.ErrCode {
	...
	if table.GetTables().PlaneRoomTable.Get().Status == 0 {
	...
	// 下注限制
	betLimit := table.GetTables().PlaneTable.Get(pos + 1).BetLimit[user.RegistArea]
	...
	t.sendCurrency(userid, (-1 * (num - cashBet)), (-1 * cashBet), int32(pb.LOG_TYPE100), fmt.Sprintf("飞机%s房间下注", t.DeskData.Rid))
...
func (t *Desk) newBiewFreeBet(userid string, num int64, pos int32) pb.ErrCode {
	...
	status := table.GetTables().PlaneRoomTable.Get().Status
	if status == 0 {
		return pb.RoomMaintenance
	}
	user := t.NewbiewRobot[userid]
	...
	betLimit := table.GetTables().PlaneTable.Get(pos + 1).BetLimit[0]
	if c, ok := t.CrashBets[userid]; ok {
		if c.GetSum()+num > int64(betLimit.Value[1]) {
			return pb.BetTopLimit //tie下注限制
		}
	}
	...
	t.DeskFree.Bets[userid] += num //个人总下注额
	//位置详细记录
	if m, ok := t.CrashBets[userid]; ok {
		m.Coin += num
		t.CrashBets[userid] = m
	} else {
	...
	_, ntf := resFreeBet(num, t.CrashBets[userid].GetSum(), t.DeskFree.Bets, user, pos, true)
	t.broadcast(ntf)
	return pb.OK
}
...
func (t *Desk) nextBetBetting() {
	for _, pos := range []int32{0, 1} {
		...
		for userid, bet := range nextBets {
			...
			ntf1 := &pb.PLANENextBetNtf{Rsp: msg}
...
// 下注消息
func resFreeBet(val, posBets int64, roomBets map[string]int64, user *data.User, pos int32, stateInBet bool) (*pb.PLANEBetRsp, *pb.PLANEBetNtf) {
	var pool, bets int64
...
func (t *Desk) freeGameOver() {
	...
	//赠送金库存变化
	if t.GiveStock > 0 {
		// t.changeStock(t.GiveStock, 0, 0, 0, 0, 0, "80033")
	}
}
...
func (t *Desk) gameSettlement() {
	...
	for k, c := range crashScoreMap {
		...
		if !role.Robot {
			...
			// 人机逃脱了倍数大于10才记录(plane大赢家榜)
			t.crashDetail(role, score, mcash, 0, 0, 0)
		}

		if !role.GetRobot() {
			// 事件(非机器人)
			bean := &event.GameRecordEvent{Gtype: uint32(pb.PLANE), Win: score > 0}
...
// 保存上一局记录
func (t *Desk) freeLastRoundRecord() {
	lastRecord := new(pb.CrashLastRoundRecord)
	lastRecord.BetRank = t.packFreeBetRank(100)
	lastRecord.Multiple = t.CRASHDeskFree.CRASHMulpitle
	for _, bet := range t.DeskFree.Bets {
		lastRecord.BetAmount += bet
	}
	lastRecord.Players = int32(len(t.DeskFree.Bets))
	t.CRASHDeskFree.LastRecord = lastRecord
}

// 输赢记录
func (t *Desk) _CRASHRecord(useid string, num int64) {
	bets := t.DeskFree.Bets[useid]
	msg := &pb.FreeSetRecord{Gtype: int32(pb.PLANE), Score: num, Bet: bets}
	role := t.roles[useid]
	var result int32 = 1
	if num > 0 {
		// 赢了
	...
	if w, ok := role.FreeWinMap[int32(pb.PLANE)]; ok {
		wins = w
	}
	...
	role.FreeWinMap[int32(pb.PLANE)] = wins
	...
	role.RoundGames[int32(pb.PLANE)]++
	if role.RoundGames[int32(pb.PLANE)] <= 1 {
	...
	if !role.Robot {
		...
		if err = mq.NatsPublish(mq.TopicGameBets, &pb.PublishGameBets{
			...
			glog.Error("publish user plane bets error", err)
		}

		log := handler.GameFlowWaterLog(bets, int(pb.PLANE), 0, len(role.RechargeTarge), useid)
...
func (t *Desk) CRASHRecord(userid string, num int64) {
	...
	if !role.Robot {
		...
		log := handler.GameFlowWaterLog(bets, int(pb.PLANE), 0, len(role.RechargeTarge), userid)
		myactor.Logger().Tell(log)
	}
}
...
// ' 游戏结束消息
func (t *Desk) resOverFree() *pb.PLANEGameoverNtf {
	msg := &pb.PLANEGameoverNtf{
		State:    t.state,
...
func (t *Desk) WinScoreSettlement(userid string, score data.Currency) {
	...
	// 	t.controlSettlement(userid, score.Diamond)
	// }
}
...
func (t *Desk) _takeoff() {
	...
	t.state = int32(pb.STATE_LEAD)
	// glog.Infof("plane factor %.2f, stock:%s, gameId:%s, maxMulpitle:%d", factor, room.Id, t.GameId, t.CRASHControlMulpitle)
}
...
func (t *Desk) eventPost(userid string, eventId int32, bean any) {
	...
	if err != nil {
		glog.Errorf("PLANE event Marshal fail")
		return
	}
...
func (t *Desk) boomPoint() {
	...
	// 		continue
	// 	}
	// 	ntf := &pb.PLANEMultipleNtf{Multiple: t.CRASHMulpitle}
	// 	// 当前返奖额
	// 	if c, ok := crashBets[userid]; ok {
...
func (t *Desk) back(userid string, auto bool, multiple, pos int32) {
	...
	// 返奖上限
	winLimit := table.GetTables().PlaneTable.Get(pos + 1).WinLimit[0]
	...
	var mcash int64
	if win.GetSum() > 0 {
		taxRate := table.GetTables().PlaneRoomTable.Get().TaxRate[role.RegistArea]
		if taxRate > 0 {
			mcash = int64(math.Ceil(float64(win.Diamond) * float64(taxRate) / 10000))
			win.Sub(data.Currency{Diamond: mcash})
		}
	}
	t.sendCurrency(userid, bets.Coin+win.Coin, bets.Diamond+win.Diamond, int32(pb.LOG_TYPE101), fmt.Sprintf("飞机房间%s位置%d撤离", t.DeskData.Rid, pos+1))
	if bets.Out > 0 {
...
func (t *Desk) _getNewbiewMulpitle() int32 {
	...
	max := t.Game.PLANE.MaxWithdrawable[0] // A类可提现金额上限
	userid := t.GetOnlyOnePlayer()
	if userid != "" {
		role := t.roles[userid]
		if role.IsB() {
			// B类
			max = t.Game.PLANE.MaxWithdrawable[1]
		}
	}
	bet := data.Currency{}
