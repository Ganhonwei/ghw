...
func (a *Desk) checkMarquee(userid string, score data.Currency) {
	// PLANE := a.DeskData.Game.PLANE
	// if score.GetSum() >= PLANE.Msg_Score {
	// 	if s, ok := a.roles[userid]; ok {
	// 		marquee := &pb.MarQueeNtf{
	// 			Userid:  userid,
	// 			Content: handler.BuildPLANEMarquee(s.Nickname, score.GetSum()),
	// 		}
...
func (a *Desk) cfgWinner() {
	if env == "dev" {
		...
		if boomMulpitle != "" {
			...
			if err == nil {
				a.CRASHControlMulpitle = int32(b)
			}
		}
	}
}
