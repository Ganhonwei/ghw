...
func (t *Desk) _robotHandler(userid string) {
	...
	// 配置
	robot := t.Game.PLANE.Robot
	for {
		if robot.RobotLeave == nil {
			continue
		}
		ntf := &pb.PLANERobotStrategyNtf{
...
func (t *Desk) robotBet() {
	...
	robotTable := table.GetTables().PlaneRobotTable.Get()
	...
	chips0 := table.GetTables().PlaneTable.Get(1).Chips[0]
	chips1 := table.GetTables().PlaneTable.Get(2).Chips[0]
...
func (t *Desk) _robotSettlement(id string, c data.Currency) bool {
	if user, ok := t.NewbiewRobot[id]; ok {
		if t.CRASHMulpitle == 5000 {
			m := float64(utils.RandInt32N(t.Game.PLANE.Jackpot[1]-t.Game.PLANE.Jackpot[0]) + t.Game.PLANE.Jackpot[0])
