...
func (a *DeskActor) enterFreeDesk(arg *pb.EnterDesk, ctx actor.Context) {
	...
	count := uint32(table.GetTables().PlaneRoomTable.Get().Count) // 房间人数
	//查找房间
	...
	case int32(pb.ROOM_TYPE2): //百人
		planeId := pb.GameType_name[int32(pb.PLANE)]
		gameData := handler.NewFreeGameData(a.Name, planeId, int32(pb.PLANE), int32(pb.DESK_TYPE_NORMAL))
		gameData.Count = uint32(table.GetTables().PlaneRoomTable.Get().Count)
...
func (a *DeskActor) _enterDesk(arg *pb.EnterDesk, ctx actor.Context) {
		...
		if arg.Dtype == int32(pb.DESK_TYPE_POINTCONTROL) {
			...
			if r, ok := rsp.(*pb.EnteredDesk); ok {
				if r.Error == pb.OK {
					arg.Sender.Tell(r)
					// 新建一张桌子
					gameData := handler.NewFreeGameData(a.Name, "80023", int32(pb.PLANE), int32(pb.DESK_TYPE_POINTCONTROL))
	...
	case int32(pb.ROOM_TYPE2): //百人
		//TODO 优化为后台添加配置
		gameData := handler.NewFreeGameData(a.Name, "80011", int32(pb.PLANE), int32(pb.DESK_TYPE_NORMAL))
		switch arg.Dtype {
		case int32(pb.DESK_TYPE_NEWBIEW):
			gameData = handler.NewFreeGameData(a.Name, "80011", int32(pb.PLANE), int32(pb.DESK_TYPE_NEWBIEW))
		case int32(pb.DESK_TYPE_POINTCONTROL):
			gameData = handler.NewFreeGameData(a.Name, "80023", int32(pb.PLANE), int32(pb.DESK_TYPE_POINTCONTROL))
		case int32(pb.DESK_TYPE_NORMAL_B):
			gameData = handler.NewFreeGameData(a.Name, "80011", int32(pb.PLANE), int32(pb.DESK_TYPE_NORMAL_B))
		}
		if deskPid, ok := a.spawnDesk(gameData, ctx); ok {
			deskPid.Tell(arg)
			return
	...
	case int32(pb.ROOM_TYPE0): //自由
		...
		gameData := handler.NewCoinGameData(a.Name,
			int32(pb.PLANE), arg.Dtype, arg.Ltype)
		if deskPid, ok := a.spawnDesk(gameData, ctx); ok {
			deskPid.Tell(arg)
			return
		}
...
func (a *DeskActor) createPCRoom(ctx actor.Context) {
	...
	if num <= 0 {
		...
		gameData := handler.NewFreeGameData(a.Name, "80023", int32(pb.PLANE), int32(pb.DESK_TYPE_POINTCONTROL))
		if deskPid, ok := a.spawnDesk(gameData, ctx); ok {
			glog.Infof("deskPid: %s, gameid:80023, gtype:%d", deskPid.String(), pb.PLANE)
			// return
		}
	}
}
