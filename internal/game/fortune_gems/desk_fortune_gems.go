...
import (
	"fmt"
	"goserver/gen/pb"
	fortunegems "goserver/internal/global/rtp/fortune_gems"
	"goserver/pkg/data"
...
func (t *Desk) changeDeskState(state pb.DeskState) {
	...
	// ntf := &pb.FortuneGemsPushStateNtf{State: state}
...
func (t *Desk) checkStepSettlementForce() {
	...
	// 倒计时10s通知
	if restTimer == 100 {
		// t.send2user(&pb.FortuneGemsCountDownNtf{ForceSettleTime: time.Now().Unix() + int64(restTimer/10)})
		return
	}
	// 强制结算
...
// mines 下注
func (t *Desk) fortuneGemsBet(userid string, arg *pb.FortuneGemsBetReq) (rsp *pb.FortuneGemsBetRsp) {
	...
	// 注单号
	orderID, err := myorderid.GenerateOrderID("0109")
	if err != nil {
		rsp.Error = pb.Failed
		return
	}
	...
	t.detail = &data.Detail{
		...
		WaterId:           orderID,
		RealGame:          t.role != nil,
		FortuneGemsDetail: &data.FortuneGemsDetail{},
	}
	detail := t.detail.FortuneGemsDetail
	detail.BeforeScore = t.role.GetScore()
	detail.BeforeCash = t.role.Diamond

	inputReels := make(map[int32]fortunegems.Symbol)
	for k, v := range arg.Reels {
		inputReels[k] = fortunegems.Symbol(v)
	}

	score, reels, specificSymbol, winLines, winMultiplier, normalWin, valid := fortunegems.PlaySlot(bet, arg.ExtraBet, inputReels, user.Userid)
	...
	//score为实际扣分
	t.sendCurrency(userid, 0, (-1 * score), int32(pb.LOG_TYPE155), fmt.Sprintf("FortuneGems房间%s下注", t.DeskData.Rid))

	rsp.Error = pb.OK
	rsp.Bet = bet
	rsp.Score = score
	rsp.Reels = map[int32]pb.FortuneGemsSymbol{}
	for k, v := range reels {
		rsp.Reels[k] = pb.FortuneGemsSymbol(v)
	}
	rsp.SpecificSymbol = pb.FortuneGemsSymbol(specificSymbol)
	rsp.WinLines = winLines
	rsp.WinMultiplier = winMultiplier
	rsp.NormalWin = normalWin
	rsp.TotalWin = normalWin
	rsp.OrderID = orderID

	if rsp.TotalWin > 0 {
		t.sendCurrency(userid, 0, rsp.TotalWin, int32(pb.LOG_TYPE156), fmt.Sprintf("FortuneGems房间%s返奖", t.DeskData.Rid))
	}
	...
	//事件
	bean := &event.GameRecordEvent{Gtype: uint32(pb.FORTUNE_GEMS), Win: winScore > 0}
	...
	// 	GameType: int32(pb.FORTUNE_GEMS),
	// 	Win:      winScore > 0,
	// 	ActTimes: t.step,
	// })
	...
	if err = mq.NatsPublish(mq.TopicGameBets, &pb.PublishGameBets{
		...
		glog.Error("publish user fortune_gems bets error", err)
	}
	...
	detail.Score = score
	detail.Reels = map[int32]pb.FortuneGemsSymbol{}
	for k, v := range reels {
		detail.Reels[k] = pb.FortuneGemsSymbol(v)
	}
	detail.SpecificSymbol = pb.FortuneGemsSymbol(specificSymbol)
	detail.WinLines = winLines
	detail.WinMultiplier = winMultiplier
	detail.NormalWin = normalWin
	detail.TotalWin = normalWin
