...
func (t *Desk) gameSettlement(force bool) {
	...
	// }
	// msg := &pb.FortuneGemsGameoverNtf{
	// 	Pits:         t.minesPitsUser,
	// 	User:         t.fortuneGemsUserDataMsg(),
...
// 进入房间响应消息
func (t *Desk) fortuneGemsEnterMsg(userid string) *pb.FortuneGemsEnterRoomRsp {
	msg := new(pb.FortuneGemsEnterRoomRsp)
	if t.role == nil || t.role.Userid != userid {
	...
	//房间数据
	msg.Roominfo = handler.PackFortuneGemsRoom(t.DeskData)
	t.fortuneGemsRoomDataMsg(msg.Roominfo)
	...
	msg.Userinfo = t.fortuneGemsUserDataMsg()
	return msg
}

func (t *Desk) fortuneGemsRoomDataMsg(room *pb.FortuneGemsRoomData) {
	// room.State = t.state
	// room.Bets = t.betNum
	// room.Mines = t.mines
	...
	// 	rule := &pb.FortuneGemsStepRule{Mines: step.Mines}
	// 	var i int32 = 0
	// 	for ; i < 25; i++ {
...
func (t *Desk) fortuneGemsUserDataMsg() (msg *pb.FortuneGemsRoomUser) {
	msg = new(pb.FortuneGemsRoomUser)
	if v := t.role; v != nil {
		msg.Userid = v.Userid
