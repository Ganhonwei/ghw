...
func (t *Desk) resumeGame(reason int) {
	...
	case PAUSE_REASON_1:
		// 比牌输的要不要离开
		if biLoseSeat, ok := t.pauseArg.(uint32); ok {
			t.robotLeaveWhenFinish(biLoseSeat, false)
		}
		t.setNextActSeat()
	case PAUSE_REASON_2:
		t.allBi()
	...
	case PAUSE_REASON_4:
		...
		for seatid, seat := range t.seats {
			if t.isRobot(seatid) && !seat.TryLeave {
				t.robotLeaveWhenFinish(seatid, seat.LeaveWin)
			}
		}
...
func (t *Desk) check() {
	...
	// 新手试探/稳定局牌型
	newbie := t.newbieProbe()
	if !newbie || t.cardTypeId == 0 {
	...
	num := utils.RandMN(int(conf.RobotNum[0]), int(conf.RobotNum[1]))
	t.robotNum = num
}
	...
	case int32(pb.STATE_CHARGE):
		if len(t.chargingSeats) > 0 && t.isRobot(t.chargingSeats[0]) {
			...
			if t.timer >= t.robotChargingTime {
				...
				zlog.Infof("%s robot chargeInGame finish: %s", t.GameId, userid)
...
func (t *Desk) _changeStock(cash_stock, bonus_stock, cash_ming, bonus_ming, cash_an, bonus_an int64) {
	...
	if t.Dtype == int32(pb.DESK_TYPE_NEWBIEW) {
		...
		m := &pb.NewbieStock{
			...
			CashMing:  cash_ming,
			CashAn:    cash_an,
			First:     role.RoundGames[int32(pb.HUA)] <= 1,
		}
		t.roomPid.Tell(m)
	}
}
...
func (t *Desk) Lose(seatid uint32) {
	...
	if !role.Robot {
		//新手试探期状态记录
		t.newbieProbeGameOver()

		// 偷鸡输赢记录
		t.hurtPlayerLog(false, userid, score_final)
		// 更新玩家vh记录
		t.updatePlayerVHRecord(false, userid)
		// 更新策略数据
		t.updatePlayerStrategy1001Heartbeat(userid)
		// 更新1003策略输赢分
		t.updatePlayerStrategy1003Score(false, userid, score_final)
	}
}
...
func (t *Desk) Win(seatid uint32) {
	...
	if t.isPrivFunRoom() {
		...
		t.shareAmount(userid, score_final)
		t.sendCurrency(userid, score_final, int32(pb.LOG_TYPE111), fmt.Sprintf("tp%s房间赢分", t.DeskData.Rid)) //加钱
	}
	t.RecordDetail(seatid, status.ActNum, score_final, status.Stock)
	...
	if !role.Robot {
		...
		// 偷鸡输赢记录
		t.hurtPlayerLog(true, userid, score_final-status.ActNum)
		// 更新玩家vh记录
		t.updatePlayerVHRecord(true, userid)
		// 更新策略数据
		t.updatePlayerStrategy1001Heartbeat(userid)
		// 更新1003策略输赢分
		t.updatePlayerStrategy1003Score(true, userid, score_final)
	}
}
...
func (t *Desk) gameOver(winner uint32) {
	...
	t.Win(winner)
	winnerSeat := t.getSeat(winner)
	winnerSeat.ShowCards = winnerSeat.Cards
...
func (t *Desk) huaRecord(seat uint32, num int64) {
	...
	msg := &pb.FreeSetRecord{Gtype: int32(pb.HUA), Score: num, Bet: bet}
	if num > 0 { //赢
		msg.Rtype = 1
	} else if num < 0 { //输
		msg.Rtype = -1
	...
	if err = mq.NatsPublish(mq.TopicGameBets, &pb.PublishGameBets{
		...
		glog.Error("publish user hua bets error", err)
	}

	// 打码量日志
	log := handler.GameFlowWaterLog(bet, int(pb.HUA), 0, len(role.RechargeTarge), userid)
