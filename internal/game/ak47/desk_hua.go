...
// '进入房间响应消息
func (t *Desk) privEnterMsg(userid string) *pb.AK47EnterRoomRsp {
	msg := new(pb.AK47EnterRoomRsp)
	//房间数据
...
// 投票信息
func (t *Desk) voteInfoMsg() (msg *pb.AK47RoomVote) {
	msg = new(pb.AK47RoomVote)
	if t.DeskPriv != nil {
...
// 发起投票
func (t *Desk) launchVote(userid string, vote uint32) (msg *pb.AK47LaunchVoteRsp) {
	msg = new(pb.AK47LaunchVoteRsp)
	errcode := t.checkVote()
	if errcode != pb.OK {
		msg.Error = errcode
		return
...
// 投票
func (t *Desk) privVote(userid string, vote uint32) (msg *pb.AK47VoteRsp) {
	msg = new(pb.AK47VoteRsp)
	errcode := t.checkVote()
	if errcode != pb.OK {
		msg.Error = errcode
		return
...
func (t *Desk) WaitTooLong(seat uint32) {
	msg := new(pb.AK47CoinWaitTooLongNtf)
	msg.Seat = seat
	t.broadcast(msg)
}
...
func (t *Desk) _changeStock(cash_stock, bonus_stock, cash_ming, bonus_ming, cash_an, bonus_an int64) {
	...
	if t.Dtype == int32(pb.DESK_TYPE_NEWBIEW) {
		...
		m := &pb.NewbieStock{
			...
			CashMing:  cash_ming,
			CashAn:    cash_an,
			First:     role.RoundGames[int32(pb.AK47)] <= 1,
		}
		t.roomPid.Tell(m)
	}
}
...
// 处理点控
func (t *Desk) handlePointControl(role *data.DeskRole, ak47detail *data.AK47Detail) {
	...
	diff := ak47detail.AfterCash - ak47detail.BeforeCash
	if diff == 0 {
		return
	}
...
func (t *Desk) Lose(seatid uint32) {
	...
	over.Changecards = seat.ChangeCards
	...
	t.ak47Record(seatid, score_final)
...
func (t *Desk) Win(seatid uint32) {
	...
	// }
	t.score[seatid] = score_final
	t.sendCurrency(userid, score_final, int32(pb.LOG_TYPE115), fmt.Sprintf("ak47%s房间赢分", t.DeskData.Rid)) //加钱
...
func (t *Desk) RecordDetail(seat uint32, act_num, score_final int64, stock *data.ActStock) {
	...
	//牌型
	ak47detail.Cards = t.getHandCards(seat)
	ak47detail.ChangeCards = t.getHandChangeCards(seat)

	//总投注、底注
	ak47detail.Bet = act_num                   //总投注
	ak47detail.Bottom = int64(t.DeskData.Ante) //底注

	//库存
	if stock != nil {
		ak47detail.CashStock = stock.CashStock
		ak47detail.BonusStock = stock.BonusStock
		ak47detail.CashMingTax = stock.CashMing
		ak47detail.BonusMingTax = stock.BonusMing
		ak47detail.CashAnTax = stock.CashAn
		ak47detail.BonusAnTax = stock.BonusAn
	}

	//结算
	ak47detail.Score = score_final

	//账变后分数、彩金、奖励金
	ak47detail.AfterScore = role.GetScore()
	ak47detail.AfterCash = role.GetDiamond()
	ak47detail.AfterBonus = role.GetCoin()

	//处理点控
	t.handlePointControl(role, ak47detail)
}
...
// 输赢记录
func (t *Desk) ak47Record(seat uint32, num int64) {
	userid := t.getUserid(seat)
	role := t.getRole(userid)
	if userid == "" || role == nil {
		return
	...
	msg := &pb.FreeSetRecord{Gtype: int32(pb.AK47), Score: num, Bet: bet}
	if num > 0 { //赢
		msg.Rtype = 1
	} else if num < 0 { //输
		msg.Rtype = -1
	...
	if err = mq.NatsPublish(mq.TopicGameBets, &pb.PublishGameBets{
		...
		glog.Error("publish user ak47 bets error", err)
	}

	// 打码量日志
	log := handler.GameFlowWaterLog(bet, int(pb.AK47), 0, len(role.RechargeTarge), userid)
...
func (t *Desk) _calcStockAndTax(score int64, options ...int64) (score1, stock, ming, an int64) {
	...
	ming_tax := t.Game.AK47.MingTax
	an_tax := t.Game.AK47.AnTax
...
func (t *Desk) jiesuan2(ltype int32, score map[uint32]int64) {
	for k, v := range score {
		...
		case int32(pb.ROOM_TYPE0): //自由
			if v > 0 {
				t.sendCurrency(userid, v, ltype, fmt.Sprintf("ak47%s房间赢分", t.DeskData.Rid))
			}
		case int32(pb.ROOM_TYPE1): //私人
			if v > 0 {
