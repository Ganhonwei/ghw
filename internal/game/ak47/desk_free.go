...
// '进入房间响应消息
func (t *Desk) freeEnterMsg(userid string) *pb.AK47FreeEnterRoomRsp {
	msg := new(pb.AK47FreeEnterRoomRsp)
	//房间数据
	msg.Roominfo = handler.PackAK47FreeRoom(t.DeskData)
	msg.Roominfo.State = t.state
	//坐下玩家信息
	msg.Userinfo = t.freeSeatBetsMsg()
	//位置下注信息
	msg.Betsinfo = handler.PackAK47RoomBets(t.DeskFree.SeatBets)
	return msg
}

// 进入消息
func (t *Desk) freeCameinMsg(userid string) {
	msg := new(pb.AK47FreeCameinNtf)
	msg.Userinfo = t.freeSeatRoleMsg(userid)
	t.broadcast(msg)
}

// 位置上玩家数据
func (t *Desk) freeSeatRoleMsg(userid string) (msg *pb.AK47FreeUser) {
	if v, ok := t.roles[userid]; ok {
		if v.Seat == 0 {
			return //没有坐下不广播
		}
		msg = handler.PackAK47FreeUser(v.User)
		msg.Seat = v.Seat
		if t.DeskFree == nil {
			return
		}
...
// 所有坐下玩家数据
func (t *Desk) freeSeatBetsMsg() (msg []*pb.AK47FreeUser) {
...
// 玩家下注数据
func (t *Desk) freeBetsMsg(userid string) (msg []*pb.AK47RoomBets) {
	...
	case int32(pb.DEALER_DOWN):
		...
		case int32(pb.STATE_BET): //下注中
			if userid == t.DeskGame.Dealer {
				...
				msg := handler.BeAK47DealerMsg(0, int64(num), t.DeskGame.Dealer,
...
// '获取上庄列表消息
func (t *Desk) dealerListMsg() (msg *pb.AK47FreeDealerListRsp) {
	msg = new(pb.AK47FreeDealerListRsp)
	if !t.isFree() {
...
func resFreeBet(seat, beseat uint32, val, coin,
	bets int64, userid string) *pb.AK47FreeBetRsp {
	return &pb.AK47FreeBetRsp{
		Seat:   seat,
		Beseat: beseat,
		Value:  uint32(val),
		Coin:   coin,
...
// ' 百人输赢趋势
func (t *Desk) freeTrends() *pb.AK47FreeTrendRsp {
	msg := new(pb.AK47FreeTrendRsp)
	for _, v := range t.DeskFree.Trends {
		msg2 := &pb.AK47FreeTrend{
			Seat2: v.Seat2,
			Seat3: v.Seat3,
			Seat4: v.Seat4,
			Seat5: v.Seat5,
...
// ' 百人上局赢家列表
func (t *Desk) freeWiners() *pb.AK47FreeWinersRsp {
	msg := new(pb.AK47FreeWinersRsp)
	for _, v := range t.DeskFree.Winers {
		msg2 := &pb.AK47FreeWiner{
			Userid:   v.Userid,
			Nickname: v.Nickname,
			Photo:    v.Photo,
			Coin:     v.Coin,
...
// ' 百人玩家列表
func (t *Desk) freeRoles() *pb.AK47FreeRolesRsp {
	msg := new(pb.AK47FreeRolesRsp)
	for _, v := range t.roles {
		if v.User == nil {
			continue
		}
		msg2 := &pb.AK47FreeRole{
...
// ' 百人坐下
func (t *Desk) freeSit(userid string, arg *pb.AK47SitReq) (msg *pb.AK47SitRsp) {
	errcode := t.sitCheck(userid, arg)
	if errcode != pb.OK {
		msg = new(pb.AK47SitRsp)
		msg.Error = errcode
		return
	}
	switch arg.Type {
...
func (t *Desk) sitCheck(userid string, arg *pb.AK47SitReq) pb.ErrCode {
	user := t.getPlayer(userid)
	if user == nil {
		glog.Errorf("userid %s not exist", userid)
		return pb.NotInRoom
...
func (t *Desk) shuffle() {
	...
	feilaizi := append([]uint32{}, algo.Feilaizi...)
	laizi := append([]uint32{}, algo.Laizi...)
	...
	// 	d[i], d[j] = d[j], d[i]
	// }
	t.DeskGame.Cards = feilaizi
	t.DeskGame.LaiCards = laizi

	algo.Shuffle(t.DeskGame.LaiCards) //把所有得癞子洗一遍
}
...
func (t *Desk) freeGameOver() {
	...
	//
	var a1 uint32 = algo.HuaType(cs1) //1位置庄家牌力
	var a2 uint32 = algo.HuaType(cs2) //2位置闲家牌力
	var a3 uint32 = algo.HuaType(cs3) //3位置闲家牌力
	var a4 uint32 = algo.HuaType(cs4) //4位置闲家牌力
	var a5 uint32 = algo.HuaType(cs5) //5位置闲家牌力
...
// ' 发牌消息
func resDraw(seat uint32, state int32, cards []uint32) *pb.AK47DrawNtf {
	return &pb.AK47DrawNtf{
		Seat:  seat,
		State: state,
		Cards: cards,
	}
...
func resFreeStart(dealer, photo, nickname string, state int32,
	carry int64, dealerNum, left uint32) *pb.AK47FreeGamestartNtf {
	return &pb.AK47FreeGamestartNtf{
		Dealer:        dealer,
		Photo:         photo,
		State:         state,
		Coin:          carry,
...
// ' 游戏结束消息
func (t *Desk) resOverFree() *pb.AK47FreeGameoverNtf {
	var left uint32
	if t.DeskGame.Dealer == "" {
		left = 1
	} else {
