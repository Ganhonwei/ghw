...
func (t *Desk) userLeaveDesk(userid string) {
	if v, ok := t.roles[userid]; ok {
		//离开状态消息
		msg1 := new(pb.LHLeaveNtf)
		msg1.Userid = userid
		t.broadcast2(userid, msg1)

		// 事件
		t.eventPost(userid, event.Initiative_Leave, &event.InitiativeLeaveEvent{Gtype: int(pb.LHD)})
