...
func (t *Desk) InitDesk() {
	...
	case int32(pb.ROOM_TYPE2): //百人
		...
		choices = append(choices, utils.Choice{Weight: 7, Item: 1})
		choices = append(choices, utils.Choice{Weight: 7, Item: 2})
		choices = append(choices, utils.Choice{Weight: 2, Item: 0})
		room := t.DeskData.Game
		initScore := room.LHD.Robot.InitScore
		...
		for i := 0; i < 20; i++ {
			...
			// 自定义头像
			t.getHead(user)
			user.FreeWinMap[int32(pb.LHD)] = handler.GenNewbiewFreeWin(int32(pb.LHD))
			t.NewbiewRobot[user.Userid] = user
			c, _ := utils.WeightedChoice(choices)
			t.LHHistory = append(t.LHHistory, int32(c.Item.(int)))
		}
...
func (a *Desk) checkMarquee(userid string, score data.Currency) {
	lhd := a.DeskData.Game.LHD
	if score.GetSum() >= lhd.Msg_Score {
		if s, ok := a.roles[userid]; ok {
			marquee := &pb.MarQueeNtf{
				Userid:  userid,
				Content: handler.BuildLHMarquee(s.Nickname, score.GetSum()),
			}
