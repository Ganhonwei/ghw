...
func (a *DeskActor) enterDesk(arg *pb.EnterDesk, ctx actor.Context) {
		...
		if arg.Dtype == int32(pb.DESK_TYPE_POINTCONTROL) {
			...
			if r, ok := rsp.(*pb.EnteredDesk); ok {
				if r.Error == pb.OK {
					arg.Sender.Tell(r)
					// 新建一张桌子
					gameData := handler.NewFreeGameData(a.Name, "30023", int32(pb.LHD), int32(pb.DESK_TYPE_POINTCONTROL))
	...
	case int32(pb.ROOM_TYPE2): //百人
		//TODO 优化为后台添加配置
		gameData := handler.NewFreeGameData(a.Name, "30011", int32(pb.LHD), int32(pb.DESK_TYPE_NORMAL))
		switch arg.Dtype {
		case int32(pb.DESK_TYPE_NEWBIEW):
			gameData = handler.NewFreeGameData(a.Name, "30011", int32(pb.LHD), int32(pb.DESK_TYPE_NEWBIEW))
		case int32(pb.DESK_TYPE_POINTCONTROL):
			gameData = handler.NewFreeGameData(a.Name, "30023", int32(pb.LHD), int32(pb.DESK_TYPE_POINTCONTROL))
		case int32(pb.DESK_TYPE_NORMAL_B):
			gameData = handler.NewFreeGameData(a.Name, "30011", int32(pb.LHD), int32(pb.DESK_TYPE_NORMAL_B))
		}
	...
	case int32(pb.ROOM_TYPE1): //私人
		...
		if deskPid, ok := a.spawnDesk2(gameData, ctx); ok {
			...
			msg2 := handler.ChangeCurrencyMsg((-1 * int64(gameData.Cost)),
				0, 0, 0, 0, 0, int32(pb.LOG_TYPE2), arg.Cid, "lhd创建私人房间", "")
