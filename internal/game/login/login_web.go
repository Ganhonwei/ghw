...
func webHandler(ctx *fasthttp.RequestCtx) {
	...
	// 创建子网对象
	_, subnet, _ := net.ParseCIDR("10.234.0.0/16")

	// 检查IP是否在子网内
	ipAddr := net.ParseIP(ip)
	if !subnet.Contains(ipAddr) {
		fmt.Fprintf(ctx, "%s %s", "failed", ip)
		return
	}
	// switch ip {
	// case "127.0.0.1":
...
func heartBeat(ctx *fasthttp.RequestCtx) {
	switch string(ctx.Method()) {
	case "GET":
	default:
	...
	// res, err := nodePid.RequestFuture(new(pb.Ping), 5*time.Second).Result().
	res, err := callNode(new(pb.Ping))
	if err != nil {
		ctx.Response.SetStatusCode(fasthttp.StatusBadRequest)
		fmt.Fprintf(ctx, "%s", "failed")
		return
	...
	if m, ok := res.(*pb.Pong); !ok || m.Msg != "ok" {
		ctx.Response.SetStatusCode(fasthttp.StatusBadRequest)
		fmt.Fprintf(ctx, "%s", "failed")
		return
	}
