...
func (t *Desk) _callRobot() {
	...
	r, n := t.roleCountNum()
	if n >= int(t.Game.CRASH.Robot.Num) {
		return
	}
	...
	msg.Min = t.Game.CRASH.Robot.InitScore[0]
	msg.Max = t.Game.CRASH.Robot.InitScore[1]
	msg.Num = 1
	t.dbmsPid.Tell(msg)
}
...
func (t *Desk) _compensation() int64 {
	...
	// 修正系数
	fix := t.Game.CRASH.CoefficientFix[len(t.Game.CRASH.CoefficientFix)-1]
	for i, v := range t.Game.CRASH.FinalCoefficient {
		if t.CRASHFactor <= float64(v)/100 {
			fix = t.Game.CRASH.CoefficientFix[i]
			break
		}
	}
	lose := float64(t.Game.Stock_Expect) * (1 - t.CRASHFactor) * float64(fix) / 10000
	// 赔付区间
	loseFlow := t.Game.CRASH.LoseFlow
...
func (t *Desk) _getTax(diamond, coin int64, m bool) (int64, int64) {
	if m {
		// 明税
		return int64(math.Ceil(float64(diamond) * float64(t.DeskData.Game.CRASH.MTax) / 10000)), int64(math.Floor(float64(coin) * float64(t.DeskData.Game.CRASH.MTax) / 10000))
	}
...
func (t *Desk) calAnTax(num int64) int64 {
	...
	real := int64(math.Round(socre * float64(10000-t.DeskData.Game.CRASH.ATax) / 10000))
...
func (t *Desk) _getNewbiewBoomProbability() int32 {
	// 新手房间默认用区间一
	boomPro := t.Game.CRASH.BoomPro1
	for _, role := range t.roles {
		// 泡沫状态用区间三
		if role.State == data.FrothState {
			boomPro = t.Game.CRASH.BoomPro3
			break
		}
	}
	return boomPro
...
func (t *Desk) _getMaxLose() int32 {
	...
	return t.DeskData.Game.CRASH.LoseLimited
}
