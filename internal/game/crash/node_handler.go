...
// '进入百人或者匹配房间
func (a *DeskActor) enterFreeDesk(arg *pb.EnterDesk, ctx actor.Context) {
	...
	if arg.Roomid != "" {
		glog.Errorf("enter roomid room: %s, %v", arg.Roomid, arg)
		rsp.Error = pb.Failed
		ctx.Respond(rsp)
		return
	}

	count := uint32(table.GetTables().CrashRoomTable.Get().Count) // 房间人数
	//查找房间
	...
	case int32(pb.ROOM_TYPE2): //百人
		crashId := pb.GameType_name[int32(pb.CRASH)]
		gameData := handler.NewFreeGameData(a.Name, crashId, int32(pb.CRASH), int32(pb.DESK_TYPE_NORMAL))
		gameData.Count = uint32(table.GetTables().CrashRoomTable.Get().Count)
...
// '进入百人或者匹配房间
// Deprecated
func (a *DeskActor) _enterDesk(arg *pb.EnterDesk, ctx actor.Context) {
	rsp := new(pb.EnteredDesk)
	rsp.Gtype = arg.Gtype
	rsp.Gameid = arg.Gameid
	rsp.Rtype = arg.Rtype
	...
	for _, v := range a.desks {
		if arg.Dtype == int32(pb.DESK_TYPE_POINTCONTROL) {
			...
			// 点控桌
			rsp, err := v.Pid.RequestFuture(arg, 3*time.Second).Result()
			if err != nil {
				continue
			}
			if r, ok := rsp.(*pb.EnteredDesk); ok {
				if r.Error == pb.OK {
					arg.Sender.Tell(r)
					// 新建一张桌子
					gameData := handler.NewFreeGameData(a.Name, "50023", int32(pb.CRASH), int32(pb.DESK_TYPE_POINTCONTROL))
	...
	case int32(pb.ROOM_TYPE2): //百人
		//TODO 优化为后台添加配置
		gameData := handler.NewFreeGameData(a.Name, "50011", int32(pb.CRASH), int32(pb.DESK_TYPE_NORMAL))
		switch arg.Dtype {
		case int32(pb.DESK_TYPE_NEWBIEW):
			gameData = handler.NewFreeGameData(a.Name, "50011", int32(pb.CRASH), int32(pb.DESK_TYPE_NEWBIEW))
		case int32(pb.DESK_TYPE_POINTCONTROL):
			gameData = handler.NewFreeGameData(a.Name, "50023", int32(pb.CRASH), int32(pb.DESK_TYPE_POINTCONTROL))
		case int32(pb.DESK_TYPE_NORMAL_B):
			gameData = handler.NewFreeGameData(a.Name, "50011", int32(pb.CRASH), int32(pb.DESK_TYPE_NORMAL_B))
		}
		if deskPid, ok := a.spawnDesk(gameData, ctx); ok {
			deskPid.Tell(arg)
			return
	...
	case int32(pb.ROOM_TYPE0): //自由
		...
		gameData := handler.NewCoinGameData(a.Name,
			int32(pb.CRASH), arg.Dtype, arg.Ltype)
		if deskPid, ok := a.spawnDesk(gameData, ctx); ok {
			deskPid.Tell(arg)
			return
		}
...
// '匹配房间
func (a *DeskActor) _changeDesk(arg *pb.ChangeDesk, ctx actor.Context) {
	rsp := new(pb.ChangedDesk)
	rsp.Gtype = arg.Gtype
	count := uint32(table.GetTables().CrashRoomTable.Get().Count)
	//查找房间
	for _, v := range a.desks {
		if v.DeskData.Rtype == arg.Rtype &&
			v.DeskData.Ltype == arg.Ltype &&
	...
	case int32(pb.ROOM_TYPE1): //私人
		...
		if deskPid, ok := a.spawnDesk2(gameData, ctx); ok {
			...
			msg2 := handler.ChangeCurrencyMsg((-1 * int64(gameData.Cost)),
				0, 0, 0, 0, 0, int32(pb.LOG_TYPE2), arg.Cid, "7up创建私人房间", "")
...
func (a *DeskActor) createPCRoom(ctx actor.Context) {
	...
	if num <= 0 {
		...
		gameData := handler.NewFreeGameData(a.Name, "50023", int32(pb.CRASH), int32(pb.DESK_TYPE_POINTCONTROL))
		if deskPid, ok := a.spawnDesk(gameData, ctx); ok {
			glog.Infof("deskPid: %s, gameid:50023, gtype:%d", deskPid.String(), pb.CRASH)
			// return
		}
	}
}
