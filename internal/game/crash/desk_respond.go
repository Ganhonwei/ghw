...
func (t *Desk) nnLeave(userid string, ctx actor.Context) {
	...
	if errcode == pb.OK && err != int32(pb.LeaveEarly) {
		// 事件
		t.eventPost(userid, event.Initiative_Leave, &event.InitiativeLeaveEvent{Gtype: int(pb.CRASH)})

		//清除数据
		msg := new(pb.CRASHLeaveRsp)
		msg.Userid = userid
...
func (t *Desk) offlineMsg(userid string) {
	if v, ok := t.roles[userid]; ok && v != nil {
		msg := new(pb.CRASHPushOfflineNtf)
		msg.Userid = userid
