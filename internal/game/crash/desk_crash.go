...
func (t *Desk) kickRobot() {
	...
	case int32(pb.ROOM_TYPE0), //自由
		...
		robot := table.GetTables().CrashRobotTable.Get() // 人机策略
		for k, v := range t.roles {
			if !v.GetRobot() {
				continue
			}
		...
		for _, u := range kicks {
			...
			//离开消息
			t.broadcast(&pb.CRASHLeaveNtf{Userid: prevUserid})
			// 进入消息
...
func (t *Desk) _Boom() bool {
	boom := handler.CrashIsBoom(t.DeskFree.CRASHMulpitle, t.Game.CRASH.BackRate, 0, r) // 默认直接爆炸,在多个策略的情况下，按照顺序执行策略，前一个不爆的情况下执行再后续策略
	for _, s := range t.CRASHStrategys {
		...
		case GCYX:
			...
			// default:
			// 	return handler.CrashIsBoom(t.DeskFree.CRASHMulpitle, t.Game.CRASH.BackRate, 0)
		}
	...
	// if len(t.CRASHStrategys) <= 0 {
	// 	// 如果没有触发策略就走默认的
	// 	boom = handler.CrashIsBoom(t.DeskFree.CRASHMulpitle, t.Game.CRASH.BackRate, 0)
	// }
