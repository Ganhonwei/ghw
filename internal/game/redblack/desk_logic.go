...
func (t *Desk) InitDesk() {
	...
	case int32(pb.ROOM_TYPE2): //百人
		...
		initScore := table.GetTables().RbRoomRobotTable.Get().InitScore
		...
		for i := 0; i < 20; i++ {
			...
			// 自定义头像
			t.getHead(user)
			user.FreeWinMap[int32(pb.REDBLACK)] = handler.GenNewbiewFreeWin(int32(pb.REDBLACK))
			t.NewbiewRobot[user.Userid] = user

			// 生成记录
			t.shuffle()
			if algo.HuaCompare(t.DeskGame.Cards[:3], t.DeskGame.Cards[3:6]) {
				ctype := algo.RedBlackType(t.DeskGame.Cards[:3])
				t.RBHistory = append(t.RBHistory, data.RBHistory{CardType: ctype, Winner: RED})
			} else {
				ctype := algo.RedBlackType(t.DeskGame.Cards[3:6])
				t.RBHistory = append(t.RBHistory, data.RBHistory{CardType: ctype, Winner: BLACK})
			}
		}
	}
}
...
func (a *Desk) checkMarquee(userid string, score data.Currency) {
	...
	if score.GetSum() >= int64(base.MaqueeScore) {
		if s, ok := a.roles[userid]; ok {
			marquee := &pb.MarQueeNtf{
				Userid:  userid,
				Content: handler.BuildRBMarquee(s.Nickname, score.GetSum()),
			}
...
// 配置开什么
func (a *Desk) cfgWinner() (uint32, uint32) {
	// if a.Dtype != int32(pb.DESK_TYPE_NORMAL) {
	// 	return -1
	// }

	var redSeat, blackSeat uint32
	err := cfg.Reload()
	if err != nil {
		return a.cfgWinner()
	}
	red := cfg.Section(nodeName).Key("red").Value()
	black := cfg.Section(nodeName).Key("black").Value()
	if red != "" {
		w, err := strconv.Atoi(red)
		if err == nil {
			redSeat = uint32(w)
		}
	}
	if black != "" {
		w, err := strconv.Atoi(black)
		if err == nil {
			blackSeat = uint32(w)
		}
	}
	return redSeat, blackSeat
}
...
func (t *Desk) returnHead(u *data.User) {
	if strings.HasPrefix(u.Photo, "http") {
		...
		glog.Debugf("%s return head %d", u.Userid, id)
	}
}
