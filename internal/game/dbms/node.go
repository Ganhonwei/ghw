...
// NewRemote 启动服务
func NewRemote(bind, kind, room, role, report string) {
	...
	// reportProps := NewReportProps()
	// remote.Register(report, reportProps)
	// reportPid, err = actor.SpawnNamed(reportProps, report)
	// if err != nil {
	// glog.Fatalf("reportPid err %v", err)
	// }
	// glog.Infof("reportPid %s", reportPid.String())
}
