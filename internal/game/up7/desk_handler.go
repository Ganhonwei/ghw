...
func (a *Desk) enterDesk(arg *pb.EnterDesk, ctx actor.Context) {
	...
	if errcode == pb.OK {
		...
		if !user.Robot {
			// 重新进入房间高潮概率清零
			user.LHDStrategy.GCYX.Hp = 0
			zlog.Info("lhd gcyx enter room hp=0")
		}
	}
	//响应消息
	rsp.Roomid = a.DeskData.Rid
	rsp.Gameid = pb.GameType_name[int32(pb.SEVEN)] //a.DeskData.Game.Id
...
func (a *Desk) enterDeskCost(userid string) {
	...
	msg2 := handler.ChangeCurrencyMsg(int64(a.DeskData.Cost),
		0, 0, 0, 0, 4, int32(pb.LOG_TYPE2), userid, fmt.Sprintf("UP进入%s房间", a.DeskData.Rid), "")
