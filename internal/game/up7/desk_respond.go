...
func (t *Desk) enterCheck(user *data.User) pb.ErrCode {
	...
	//}
	count := uint32(table.GetTables().Up7RoomTable.Get().Count)
...
func (t *Desk) userLeaveDesk(userid string) {
	if v, ok := t.roles[userid]; ok {
		...
		// 事件
		t.eventPost(userid, event.Initiative_Leave, &event.InitiativeLeaveEvent{Gtype: int(pb.SEVEN)})
		//清除数据
