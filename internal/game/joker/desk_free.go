...
// '进入房间响应消息
func (t *Desk) freeEnterMsg(userid string) *pb.JOKERFreeEnterRoomRsp {
	msg := new(pb.JOKERFreeEnterRoomRsp)
	//房间数据
	msg.Roominfo = handler.PackJOKERFreeRoom(t.DeskData)
	msg.Roominfo.State = t.state
	//坐下玩家信息
	msg.Userinfo = t.freeSeatBetsMsg()
	//位置下注信息
	msg.Betsinfo = handler.PackJOKERRoomBets(t.DeskFree.SeatBets)
	return msg
}

// 进入消息
func (t *Desk) freeCameinMsg(userid string) {
	msg := new(pb.JOKERFreeCameinNtf)
	msg.Userinfo = t.freeSeatRoleMsg(userid)
	t.broadcast(msg)
}

// 位置上玩家数据
func (t *Desk) freeSeatRoleMsg(userid string) (msg *pb.JOKERFreeUser) {
	if v, ok := t.roles[userid]; ok {
		if v.Seat == 0 {
			return //没有坐下不广播
		}
		msg = handler.PackJOKERFreeUser(v.User)
		msg.Seat = v.Seat
		if t.DeskFree == nil {
			return
		}
...
// 所有坐下玩家数据
func (t *Desk) freeSeatBetsMsg() (msg []*pb.JOKERFreeUser) {
...
// 玩家下注数据
func (t *Desk) freeBetsMsg(userid string) (msg []*pb.JOKERRoomBets) {
	...
	case int32(pb.DEALER_DOWN):
		...
		case int32(pb.STATE_BET): //下注中
			if userid == t.DeskGame.Dealer {
				...
				msg := handler.BeJOKERDealerMsg(0, int64(num), t.DeskGame.Dealer,
...
// '获取上庄列表消息
func (t *Desk) dealerListMsg() (msg *pb.JOKERFreeDealerListRsp) {
	msg = new(pb.JOKERFreeDealerListRsp)
	if !t.isFree() {
...
func resFreeBet(seat, beseat uint32, val, coin,
	bets int64, userid string) *pb.JOKERFreeBetRsp {
	return &pb.JOKERFreeBetRsp{
		Seat:   seat,
		Beseat: beseat,
		Value:  uint32(val),
		Coin:   coin,
...
// ' 百人输赢趋势
func (t *Desk) freeTrends() *pb.JOKERFreeTrendRsp {
	msg := new(pb.JOKERFreeTrendRsp)
	for _, v := range t.DeskFree.Trends {
		msg2 := &pb.JOKERFreeTrend{
			Seat2: v.Seat2,
			Seat3: v.Seat3,
			Seat4: v.Seat4,
			Seat5: v.Seat5,
...
// ' 百人上局赢家列表
func (t *Desk) freeWiners() *pb.JOKERFreeWinersRsp {
	msg := new(pb.JOKERFreeWinersRsp)
	for _, v := range t.DeskFree.Winers {
		msg2 := &pb.JOKERFreeWiner{
			Userid:   v.Userid,
			Nickname: v.Nickname,
			Photo:    v.Photo,
			Coin:     v.Coin,
...
// ' 百人玩家列表
func (t *Desk) freeRoles() *pb.JOKERFreeRolesRsp {
	msg := new(pb.JOKERFreeRolesRsp)
	for _, v := range t.roles {
		if v.User == nil {
			continue
		}
		msg2 := &pb.JOKERFreeRole{
...
// ' 百人坐下
func (t *Desk) freeSit(userid string, arg *pb.JOKERSitReq) (msg *pb.JOKERSitRsp) {
	errcode := t.sitCheck(userid, arg)
	if errcode != pb.OK {
		msg = new(pb.JOKERSitRsp)
		msg.Error = errcode
		return
	}
	switch arg.Type {
...
func (t *Desk) sitCheck(userid string, arg *pb.JOKERSitReq) pb.ErrCode {
	user := t.getPlayer(userid)
	if user == nil {
		glog.Errorf("userid %s not exist", userid)
		return pb.NotInRoom
...
// ' 发牌消息
func resDraw(seat uint32, state int32, cards []uint32) *pb.JOKERDrawNtf {
	return &pb.JOKERDrawNtf{
		Seat:  seat,
		State: state,
		Cards: cards,
	}
...
func resFreeStart(dealer, photo, nickname string, state int32,
	carry int64, dealerNum, left uint32) *pb.JOKERFreeGamestartNtf {
	return &pb.JOKERFreeGamestartNtf{
		Dealer:        dealer,
		Photo:         photo,
		State:         state,
		Coin:          carry,
...
// ' 游戏结束消息
func (t *Desk) resOverFree() *pb.JOKERFreeGameoverNtf {
	var left uint32
	if t.DeskGame.Dealer == "" {
		left = 1
	} else {
