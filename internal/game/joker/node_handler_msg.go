...
func (a *DeskActor) GetRoomList(ctx actor.Context) {
	...
	//响应
	rsp := handler.PackJOKERRoomList(arg, a.desks)
	arg.Sender.Tell(rsp)
}
...
func (a *DeskActor) createDesk(arg *pb.CreateDesk, ctx actor.Context) {
	...
	case int32(pb.ROOM_TYPE1): //私人
		...
		if deskPid, ok := a.spawnDesk2(gameData, ctx); ok {
			//创建成功消息
			msg := handler.PackJOKERCreateMsg(gameData)
			arg.Sender.Tell(msg)
			//扣除玩家消耗
			msg2 := handler.ChangeCurrencyMsg((-1 * int64(gameData.Cost)),
				0, 0, 0, 0, 0, int32(pb.LOG_TYPE2), arg.Cid, "JOKER创建房间", "")
...
func (a *DeskActor) resetMatching() {
	...
	a.matchTimer = 0
	glog.Infof("JOKER mathch reset time：%v", utils.LocalTime().Format("2006-01-02 15:04:05"))
