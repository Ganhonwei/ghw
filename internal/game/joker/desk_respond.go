...
func (t *Desk) chatText(arg *pb.ChatTextReq, ctx actor.Context) {
	...
	// 	}
	// 	if vip.EmojiCost > 0 {
	// 		t.sendCurrency(userid, -int64(vip.EmojiCost), int32(pb.LOG_TYPE102), "joker发表情")
	// 	}
	// }
...
func (t *Desk) userLeaveDesk(userid string, args ...pb.ErrCode) {
	...
	if v, ok := t.roles[userid]; ok {
		//离开状态消息
		msg1 := new(pb.JOKERLeaveRsp)
		msg1.Seat = v.Seat
		msg1.Userid = userid
		t.send2userid(userid, msg1)

		//广播
		msg2 := new(pb.JOKERLeaveNtf)
		msg2.Seat = v.Seat
		msg2.Userid = userid
		msg2.Error = err
...
func (t *Desk) offlineMsg(userid string) {
	if v, ok := t.roles[userid]; ok && v != nil {
		msg := new(pb.JOKERPushOfflineNtf)
		msg.Seat = v.Seat
		msg.Userid = userid
...
func (t *Desk) addBeDealer(userid string, st int32,
	...
	msg := handler.BeJOKERDealerMsg(st, num, t.DeskGame.Dealer,
...
func (t *Desk) delBeDealer(userid string, user *data.User) {
	...
	msg := handler.BeJOKERDealerMsg(0, num, t.DeskGame.Dealer,
		userid, user.GetNickname(), user.GetPhoto())
	t.broadcast(msg)
}
...
func (t *Desk) beComeDealer() {
	...
	msg := handler.BeJOKERDealerMsg(1, num, t.DeskGame.Dealer,
		userid, user.GetNickname(), user.GetPhoto())
	t.broadcast(msg)
}
...
// '玩家准备
func (t *Desk) readying(userid string, ready bool) (msg *pb.JOKERReadyRsp) {
	msg = new(pb.JOKERReadyRsp)
	if t.isFree() {
		msg.Error = pb.OperateError
		return
	}
...
func (t *Desk) turnDetail(userid string, action string) {
	...
	jokerdeitalturn := jokerdetail.FindJOKERDetailTurn(int(t.ActTimes))
	if jokerdeitalturn == nil {
		return
	}
	jokerdeitalturn.Operation += action
	jokerdeitalturn.Operation += ","
}
...
// 获取局内充值信息
func (t *Desk) getChargeInfo(seatid uint32) *pb.JOKERChargeInGameInfo {
	seat := t.getSeat(seatid)
	if seat == nil {
		return nil
	}
	info := &pb.JOKERChargeInGameInfo{}
...
func (t *Desk) coinCall(userid string) {
	...
	msg2 := new(pb.JOKERCoinCallNtf)

	msg2.Info = t.setBet(seatid, userid, num, fmt.Sprintf("joker%s房间跟注", t.DeskData.Rid))
	t.recordBet(num)
...
func (t *Desk) coinRaise(userid string) {
	...
	// }
	//广播消息
	msg2 := new(pb.JOKERCoinRaiseNtf)
	msg2.Info = t.setBet(seatid, userid, num, fmt.Sprintf("joker%s房间加注", t.DeskData.Rid))
	t.recordBet(num)
	msg2.Pot = t.DeskGame.BetNum
	if !t.isSee(seatid) {
		msg2.State |= int32(pb.ACT_BLIND)
...
func (t *Desk) StrategyShowCard() bool {
	...
	for _, v := range seats {
		...
		if seat.Identity == "a" && status.Pack {
			seata = seat
			t.DeskGame.Cards = append(t.DeskGame.Cards, seat.Cards[0], seat.Cards[1])
		}
	...
	if seata != nil {
		...
		seata.ShowCards, t.DeskGame.Cards = algo.GetJokerCard(cardtype, t.DeskGame.Cards)
		change = true
	}
	...
	if seatb != nil {
		...
		ch, _ := utils.WeightedChoice(choices)
		cardtype := ch.Item.(uint32)
		seatb.ShowCards, t.DeskGame.Cards = algo.GetJokerCard(cardtype, t.DeskGame.Cards)
		change = true
	}

	if seata != nil && seatb != nil {
		if algo.JokerCompare(seatb.ShowCards, seata.ShowCards) {
...
func (t *Desk) coinReplyBi(userid string, agree bool) {
	if agree { //同意比牌
		msg := new(pb.JOKERCoinReplyBiRsp)
		t.send2userid(userid, msg)

		msg2 := new(pb.JOKERCoinBiNtf)
		...
		msg1 := new(pb.JOKERCoinReplyBiRsp)
		t.send2userid(userid, msg1)

		next := t.getNextActSeat()

		msg := new(pb.JOKERCoinReplyBiNtf)
		msg.Seat = next
...
func (t *Desk) coinBi(userid string) {
	...
	if t.DeskAct.ActState&int32(pb.ACT_SHOW) == int32(pb.ACT_SHOW) { //show
		...
		msg3 := new(pb.JOKERCoinCallNtf)
		msg3.Info = t.setBet(seatid, userid, num, fmt.Sprintf("joker%s房间show", t.DeskData.Rid))
		t.recordBet(num)
		...
		// num := t.ActAnte * 2
		msg2 := new(pb.JOKERCoinCallNtf)
		msg2.Info = t.setBet(seat, userid, num, fmt.Sprintf("joker%s房间sideshow", t.DeskData.Rid))
		t.recordBet(num)
		msg2.Pot = t.DeskGame.BetNum
		t.broadcast(msg2)
...
func (t *Desk) getRechargeAmount(seat uint32) (uint32, uint32) {
	if t.isStrategy && t.chargeAmount != 0 {
		...
		configs := t.Game.JOKER.RoomRecharge
		gives := t.Game.JOKER.RoomRechargeGive
		if len(configs) <= 0 {
			return 0, 0
		}
