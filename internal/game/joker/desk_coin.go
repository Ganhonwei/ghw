...
// 进入房间响应消息
func (t *Desk) coinEnterMsg(userid string) *pb.JOKERCoinEnterRoomRsp {
	msg := new(pb.JOKERCoinEnterRoomRsp)
	//房间数据
...
func (t *Desk) randRobotNum() int {
	return utils.RandMN(t.Game.JOKER.Single_Robot[0], t.Game.JOKER.Single_Robot[1])
}
...
// 位置上玩家数据
func (t *Desk) coinRoleMsg(userid string) (msg *pb.JOKERRoomUser) {
	if v, ok := t.roles[userid]; ok {
		if v.Seat == 0 {
			return //没有坐下不广播
		}
		msg = handler.PackJOKERCoinUser(v.User)
		msg.Seat = v.Seat
...
// 所有坐下玩家数据
func (t *Desk) coinSeatBetsMsg(userid string) (msg []*pb.JOKERRoomUser) {
...
// 玩家下注数据
func (t *Desk) coinBetsMsg() (msg []*pb.JOKERRoomBets) {
	for k, v := range t.seats {
...
func (t *Desk) CheckFoamStateCardType() (ok bool, cardType int) {
	...
	if role.User.State == 4 {
		return true, t.Game.JOKER.NewbieMode.FoamCardType
	} else {
		return false, 0
	}
}
...
func (t *Desk) CheckSpecialRound(role *data.DeskRole) (ok bool, cardType int) {
	...
	var idx int
	for k, v := range t.Game.JOKER.NewbieMode.SpecialRound {
		if int(round) <= v {
			idx = k
			ok = true
			break
	...
	if ok {
		return ok, t.Game.JOKER.NewbieMode.SpecialRoundCardType[idx]
	} else {
		return false, 0
	}
}
		...
		if r, ok := t.roles[v.Userid]; ok {
			if !r.Robot {
				...
				if r.State == 3 {
					...
					index := 0
					for i, v := range t.Game.JOKER.NewbieMode.CivilianCanWithdrawRange {
						if diamond < int64(v) {
							index = i
							break
						} else {
					...
					if index > len(t.Game.JOKER.NewbieMode.CivilianCanWithdrawRange)-1 {
						index = len(t.Game.JOKER.NewbieMode.CivilianCanWithdrawRange) - 1
					}
					winRate := t.Game.JOKER.NewbieMode.CivilianWinRate[index]
					win := utils.RandWan(int32(winRate))
					if win {
						weight := t.Game.JOKER.NewbieMode.CivilianWinWeight
						var choices []utils.Choice
						...
						c, _ := utils.WeightedChoice(choices)
						i := c.Item.(int)
						return t.Game.JOKER.NewbieMode.CivilianWinCardType[i]
					} else {
						weight := t.Game.JOKER.NewbieMode.CivilianLoseWeight
						var choices []utils.Choice
						for i, v := range weight {
							choices = append(choices, utils.Choice{Weight: v, Item: i})
						}
						c, _ := utils.WeightedChoice(choices)
						i := c.Item.(int)
						return t.Game.JOKER.NewbieMode.CivilianLoseCardType[i]
					}
				} else {
					diamond := r.OutDiamond
					index := 0
					for i, v := range t.Game.JOKER.NewbieMode.CanWithdrawRange {
						if diamond < int64(v) {
							index = i
							break
						} else {
					...
					if index > len(t.Game.JOKER.NewbieMode.CanWithdrawRange)-1 {
						index = len(t.Game.JOKER.NewbieMode.CanWithdrawRange) - 1
					}
					winRate := t.Game.JOKER.NewbieMode.WinRate[index]
					win := utils.RandWan(int32(winRate))
					if win {
						weight := t.Game.JOKER.NewbieMode.WinWeight
						var choices []utils.Choice
						for i, v := range weight {
							choices = append(choices, utils.Choice{Weight: v, Item: i})
						}
						c, _ := utils.WeightedChoice(choices)
						i := c.Item.(int)
						return t.Game.JOKER.NewbieMode.WinCardType[i]
					} else {
						weight := t.Game.JOKER.NewbieMode.LoseWeight
						var choices []utils.Choice
						for i, v := range weight {
							choices = append(choices, utils.Choice{Weight: v, Item: i})
						}
						c, _ := utils.WeightedChoice(choices)
						i := c.Item.(int)
						return t.Game.JOKER.NewbieMode.LoseCardType[i]
					}
				}
...
func (t *Desk) IsRoleTriggerBetChange(role *data.DeskRole, winscore1 int64, winscore2 int64) bool {
	...
	for _, v := range t.Game.JOKER.ControlType {
		min := v.ChargeRange[0]
		max := v.ChargeRange[1]
		if max == -1 {
			max = int(math.Inf(1))
...
func (t *Desk) getFinalFactor() float64 {
	...
	if t.Game.JOKER.RoomFactorSwitch == 0 {
		//玩家平均系数
...
func (t *Desk) getMaxSeat() uint32 {
	...
	for k, v := range t.seats {
		...
		if algo.JokerCompare(cs1, cs2) { //k赢
			max = k
		}
	}
	return max
...
func (t *Desk) robotHandler() {
	...
	id := t.cardTypeId
	ct := t.Game.JOKER.FindCardType(id)
	rsg := t.Game.JOKER.FindRobotStrategyGroup(ct.RobotStrategyGroup)
		...
		if role, ok := t.roles[v.Userid]; ok && role != nil {
			if role.Robot {
				if v.Core || v.SeeAndPack || v.Identity != "" {
					...
					var rs data.JOKERGameRobotStrategy

					if algo.JokerCompare(cs1, cs2) {
						switch typ {
						case algo.BaoZi:
							rs = t.Game.JOKER.FindRobotStrategy(rsg.BaoZiBigger)
						case algo.TongHuaShun:
							rs = t.Game.JOKER.FindRobotStrategy(rsg.TongHuaShunBigger)
						case algo.ShunZi:
							rs = t.Game.JOKER.FindRobotStrategy(rsg.ShunZiBigger)
						case algo.TongHua:
							rs = t.Game.JOKER.FindRobotStrategy(rsg.TongHuaBigger)
						case algo.DuiZi:
							rs = t.Game.JOKER.FindRobotStrategy(rsg.DuiZiBigger)
						}
					} else {
						switch typ {
						case algo.BaoZi:
							rs = t.Game.JOKER.FindRobotStrategy(rsg.BaoZiSmaller)
						case algo.TongHuaShun:
							rs = t.Game.JOKER.FindRobotStrategy(rsg.TongHuaShunSmaller)
						case algo.ShunZi:
							rs = t.Game.JOKER.FindRobotStrategy(rsg.ShunZiSmaller)
						case algo.TongHua:
							rs = t.Game.JOKER.FindRobotStrategy(rsg.TongHuaSmaller)
						case algo.DuiZi:
							rs = t.Game.JOKER.FindRobotStrategy(rsg.DuiZiSmaller)
						}
					}

					msg := &pb.JOKERCoinRobotStrategyNtf{}
					msg.Id = rs.Id
					for _, v := range rs.ActionWeight {
						msg.ActionWeight = append(msg.ActionWeight, &pb.JOKERCoinActionWeight{Values: v})
					}
					msg.ActionTime = rs.ActionTime
					msg.SeeWeight = rs.SeeWeight
					msg.AgreeBi = rs.AgreeBi
					for _, v := range rs.SeeTime {
						msg.SeeTime = append(msg.SeeTime, &pb.JOKERCoinSeeTime{Values: v})
					}
...
func (t *Desk) SetAllRobotSeeAndPack(bet_change bool) {
	for _, v := range t.GetAllRobot() {
		duizi, remain := algo.GetJokerDuiZiCard(t.DeskGame.Cards)
		t.DeskGame.Cards = remain
		t.SetSeeAndPackRobot(v, duizi, bet_change)
	}
}
...
func (t *Desk) BetCheckWinScore(seatid uint32) {
	...
	bigger, remain := algo.GetJokerMaxCard(maxCards, t.DeskGame.Cards)
	t.DeskGame.Cards = remain
...
func (t *Desk) Strategy100Handler(ignoreState bool) {
	...
	//触发概率
	if !utils.RandWan(int32(t.Game.JOKER.NewbieMode.CivilianTriggerStrategyProb)) {
		return
	}
	...
	// //判断累计局数
	// if role.TPTotalRound < int32(t.Game.JOKER.Strategy100Round) {
	// 	return
	// }
	...
	//给玩家发牌 AAA
	seat.Cards, t.DeskGame.Cards = algo.GetJokerCard(algo.BaoZi1, t.DeskGame.Cards)
	...
	seata.Cards, t.DeskGame.Cards = algo.GetJokerCard(cardtype, t.DeskGame.Cards)
	...
	ch, _ = utils.WeightedChoice(choices)
	cardtype = ch.Item.(uint32)
	seatb.Cards, t.DeskGame.Cards = algo.GetJokerCard(cardtype, t.DeskGame.Cards)
	...
	if c != 0 {
		...
		ch, _ = utils.WeightedChoice(choices)
		cardtype = ch.Item.(uint32)
		seatc.Cards, t.DeskGame.Cards = algo.GetJokerCard(cardtype, t.DeskGame.Cards)
	}
	...
	if d != 0 {
		...
		ch, _ = utils.WeightedChoice(choices)
		cardtype = ch.Item.(uint32)
		seatd.Cards, t.DeskGame.Cards = algo.GetJokerCard(cardtype, t.DeskGame.Cards)
	}
...
func (t *Desk) Strategy200Handler() {
	...
	if role.RegistArea == 1 {
		...
		//触发概率
		if !utils.RandWan(int32(t.Game.JOKER.NewbieMode.FoamTriggerStrategyProb)) {
			return
		}
	}
	...
	// // 判断累计局数
	// if role.TPTotalRound < int32(t.Game.JOKER.Strategy200Round) {
	// 	return
	// }
	...
	ch, _ := utils.WeightedChoice(choices)
	cardtype := ch.Item.(uint32)
	seat.Cards, t.DeskGame.Cards = algo.GetJokerCard(cardtype, t.DeskGame.Cards)
	...
	//主机a一定比玩家小
	if algo.JokerCompare(seata.Cards, seat.Cards) {
...
func (t *Desk) Strategy300Handler() {
	...
	// 判断累计局数
	if role.TPTotalRound < int32(t.Game.JOKER.Strategy300Round) {
		return
	}
	...
	if utils.RandWan(6000) { //主机a比玩家大
		if algo.JokerCompare(seat.Cards, seata.Cards) {
			seat.Cards, seata.Cards = seata.Cards, seat.Cards
		}
	} else { //主机a比玩家小
		if algo.JokerCompare(seata.Cards, seat.Cards) {
...
func (t *Desk) detailInit() {
	...
	for k, v := range t.seats {
		...
		joker_detail := &data.JOKERDetail{}
		joker_detail.SeatId = k
		joker_detail.UserId = v.Userid
		// joker_detail.Cards = t.getHandCards(k)
		if role, ok := t.roles[v.Userid]; ok && role != nil {
			joker_detail.BeforeScore = role.GetScore()
			joker_detail.BeforeCash = role.GetDiamond()
			joker_detail.BeforeBonus = role.GetCoin()
		}
		t.detail.JOKERDetail = append(t.detail.JOKERDetail, joker_detail)
	}
}
	...
	case int32(pb.ROOM_TYPE0), //自由
		...
		if userid != "" {
			...
			if role.RegistArea == 0 {
				// 切换为A类新手配置
				t.Game.JOKER.NewbieMode = t.Game.JOKER.ANewbieMode
			}
		}
		t.Ante = uint32(t.Game.JOKER.Bottom)
		t.score = make(map[uint32]int64)
		t.over = make(map[uint32]*pb.JOKERCoinOver)
...
func (t *Desk) SetChargeCondition() {
	...
	if t.isCharge {
		ante_factor := t.Game.JOKER.AnteFactor
		// upper_factor := 1000
...
func (t *Desk) findCardTypeId() int {
	...
	index := 0
	for i, v := range t.Game.JOKER.FinalFactorRange {
		if factor_int < v {
			index = i
			break
		} else {
	...
	return t.Game.JOKER.CardTypeRange[index]
}
...
func (t *Desk) getCard(robot bool, current_cards []uint32) (cards []uint32, remain_cards []uint32) {
	...
	var choices []utils.Choice
	if !robot {
		choices = append(choices, utils.Choice{Weight: ct.PlayerWeight[0], Item: algo.BaoZi})
		choices = append(choices, utils.Choice{Weight: ct.PlayerWeight[1], Item: algo.TongHuaShun})
		choices = append(choices, utils.Choice{Weight: ct.PlayerWeight[2], Item: algo.ShunZi})
		choices = append(choices, utils.Choice{Weight: ct.PlayerWeight[3], Item: algo.TongHua})
		choices = append(choices, utils.Choice{Weight: ct.PlayerWeight[4], Item: algo.DuiZi})
		// choices = append(choices, utils.Choice{Weight: ct.PlayerWeight[5], Item: algo.GaoPai})
	} else {
		choices = append(choices, utils.Choice{Weight: ct.RobotWeight[0], Item: algo.BaoZi})
		choices = append(choices, utils.Choice{Weight: ct.RobotWeight[1], Item: algo.TongHuaShun})
		choices = append(choices, utils.Choice{Weight: ct.RobotWeight[2], Item: algo.ShunZi})
		choices = append(choices, utils.Choice{Weight: ct.RobotWeight[3], Item: algo.TongHua})
		choices = append(choices, utils.Choice{Weight: ct.RobotWeight[4], Item: algo.DuiZi})
		// choices = append(choices, utils.Choice{Weight: ct.RobotWeight[5], Item: algo.GaoPai})
	}
	c, _ := utils.WeightedChoice(choices)
	cardtype := c.Item.(uint32)

	return algo.GetJokerCard(cardtype, current_cards)
...
func (t *Desk) deal() {
	...
	// 	if algo.IsA(uint32Slice) {
	// 		card := algo.ChangeA(uint32Slice)
	// 		uint32Slice = append(uint32Slice, card)
	// 	} else if algo.IsB(uint32Slice) {
	// 		card := algo.ChangeB(uint32Slice)
	// 		uint32Slice = append(uint32Slice, card)
	// 	} else if algo.IsC(uint32Slice) {
	// 		card := algo.ChangeC(uint32Slice)
	// 		uint32Slice = append(uint32Slice, card)
	// 	} else if algo.IsD(uint32Slice) {
	// 		card := algo.ChangeD(uint32Slice)
	// 		uint32Slice = append(uint32Slice, card)
	// 	} else if algo.IsE(uint32Slice) {
	// 		card := algo.ChangeE(uint32Slice)
	// 		uint32Slice = append(uint32Slice, card)
	// 	}
...
// '自由场结算消息
func (t *Desk) resCoinOver(score map[uint32]int64) (msg *pb.JOKERCoinGameoverNtf) {
	t.state = int32(pb.STATE_OVER)
	msg = &pb.JOKERCoinGameoverNtf{
		Dealer: t.DeskGame.Dealer,
		State:  t.state,
	}
	for _, v := range t.over {
		if t.Game.JOKER.ShowSwitch == 0 {
			v.Cards = []uint32{}
		} else if !v.Show {
			v.Cards = []uint32{}
		} else {
...
// '私人局结算消息
func (t *Desk) resOver(score map[uint32]int64) (msg *pb.JOKERGameoverNtf) {
	msg = &pb.JOKERGameoverNtf{
		Dealer:     t.DeskGame.Dealer,
		DealerSeat: t.DeskGame.DealerSeat,
...
func (t *Desk) forceSee() {
	...
	for k, v := range t.seats {
		...
		if act, ok := t.DeskAct.ActSeats[k]; ok && act != nil {
			...
			act.See = true
			msg := new(pb.JOKERCoinSeeRsp)
			msg.Cards = v.Cards
			msg.Typ = algo.HuaType(msg.Cards)
			t.send2userid(v.Userid, msg)

			// 广播看牌不用All功能
			msg2 := new(pb.JOKERCoinSeeNtf)
			msg2.Seat = k
...
func (t *Desk) setNextActSeat() {
	...
	curr := t.getNextActSeat()
	//总下注额达到底池上限结束
	if t.DeskGame.BetNum >= int64(t.Game.JOKER.Pool_Limit) {
		//等待播放下注动画(最后一个下注的人)
...
func (t *Desk) raiseAnte() {
	msg := new(pb.JOKERCoinRaiseAnteNtf)
	for k, v := range t.seats {
		if !v.Ready {
			continue
		}
		...
		// t.DeskAct.ActSeats[k] = d
		//开始时下暗注
		info := t.setBet(k, v.Userid, int64(t.DeskData.Ante), fmt.Sprintf("joker%s房间下底注", t.DeskData.Rid))
		msg.Info = append(msg.Info, info)
...
// 下注成功设置
func (t *Desk) setBet(seat uint32, userid string, num int64, desc string) *pb.JOKERCoinRaiseInfo {
...
// 开始时下暗注
func (t *Desk) setBetMsg(seat uint32, userid string, num int64, desc string) *pb.JOKERCoinRaiseInfo {
	msg := &pb.JOKERCoinRaiseInfo{
		Seat:   seat,
		Userid: userid,
		Value:  num, //下底注
		// Total:  num, //总下注
	...
	switch t.DeskData.Rtype {
	case int32(pb.ROOM_TYPE0): //自由
		t.sendCurrency(userid, (-1 * num), int32(pb.LOG_TYPE112), desc)
	case int32(pb.ROOM_TYPE1): //私人
...
func (t *Desk) hedgeCard() {
	...
	if changeCard {
		...
		baozi, cards := algo.GetJokerCard(algo.BaoZi, t.DeskGame.Cards)
		copy(t.DeskGame.Cards, cards)
		ths, cards := algo.GetJokerCard(algo.TongHuaShun, t.DeskGame.Cards)
		copy(t.DeskGame.Cards, cards)
		s := t.getSeat(maxSeat)
		s.Cards = ths
		s = t.getSeat(minSeat)
