...
func (t *Desk) gameSettlement(force bool) {
	...
	// 事件(非机器人)
	bean := &event.GameRecordEvent{Gtype: uint32(pb.MINES), Win: winScore > 0}
	...
	// }
	msg := &pb.MinesGameoverNtf{
		Pits:         t.minesPitsUser,
		User:         t.minesUserDataMsg(),
...
// 进入房间响应消息
func (t *Desk) minesEnterMsg(userid string) *pb.MinesEnterRoomRsp {
	msg := new(pb.MinesEnterRoomRsp)
	if t.role == nil || t.role.Userid != userid {
	...
	//房间数据
	msg.Roominfo = handler.PackMinesRoom(t.DeskData)
	t.minesRoomDataMsg(msg.Roominfo)
	...
	msg.Userinfo = t.minesUserDataMsg()
	return msg
}

func (t *Desk) minesRoomDataMsg(room *pb.MinesRoomData) {
	room.State = t.state
	room.Bets = t.betNum
	room.Mines = t.mines
	...
	for _, step := range steps {
		rule := &pb.MinesStepRule{Mines: step.Mines}
		var i int32 = 0
		for ; i < 25; i++ {
...
func (t *Desk) minesUserDataMsg() (msg *pb.MinesRoomUser) {
	msg = new(pb.MinesRoomUser)
	if v := t.role; v != nil {
		msg.Userid = v.Userid
