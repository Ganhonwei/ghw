...
func (t *Desk) nnLeave(ctx actor.Context) {
	msg := new(pb.FortuneGems2LeaveRsp)
	role := t.getRole()
	if role == nil {
	...
	// 事件
	t.eventPost(event.Initiative_Leave, &event.InitiativeLeaveEvent{Gtype: int(pb.FORTUNE_GEMS2)})
...
func (t *Desk) notifyGateUserLeft(err pb.ErrCode, reason int32) {
	if v := t.role; v != nil && v.Pid != nil {
		...
		msg := new(pb.LeftDesk)
		msg.Error = err
		msg.Gtype = int32(pb.FORTUNE_GEMS2)
		msg.Reason = reason
		v.Pid.Tell(msg)
		// }
	}
