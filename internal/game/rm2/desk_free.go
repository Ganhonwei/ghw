...
// '进入房间响应消息
func (t *Desk) freeEnterMsg(userid string) *pb.RMFreeEnterRoomRsp {
	msg := new(pb.RMFreeEnterRoomRsp)
	//房间数据
	msg.Roominfo = handler.PackRMFreeRoom(t.DeskData)
	msg.Roominfo.State = t.state
	//坐下玩家信息
	msg.Userinfo = t.freeSeatBetsMsg()
	//位置下注信息
	msg.Betsinfo = handler.PackRMRoomBets(t.DeskFree.SeatBets)
	return msg
}

// 进入消息
func (t *Desk) freeCameinMsg(userid string) {
	msg := new(pb.RMFreeCameinNtf)
	msg.Userinfo = t.freeSeatRoleMsg(userid)
	t.broadcast(msg)
}

// 位置上玩家数据
func (t *Desk) freeSeatRoleMsg(userid string) (msg *pb.RMFreeUser) {
	if v, ok := t.roles[userid]; ok {
		if v.Seat == 0 {
			return //没有坐下不广播
		}
		msg = handler.PackRMFreeUser(v.User)
		msg.Seat = v.Seat
		if t.DeskFree == nil {
			return
		}
...
// 所有坐下玩家数据
func (t *Desk) freeSeatBetsMsg() (msg []*pb.RMFreeUser) {
...
// 玩家下注数据
func (t *Desk) freeBetsMsg(userid string) (msg []*pb.RMRoomBets) {
	...
	case int32(pb.DEALER_DOWN):
		...
		case int32(pb.STATE_BET): //下注中
			if userid == t.DeskGame.Dealer {
				...
				msg := handler.BeRMDealerMsg(0, int64(num), t.DeskGame.Dealer,
...
// '获取上庄列表消息
func (t *Desk) dealerListMsg() (msg *pb.RMFreeDealerListRsp) {
	msg = new(pb.RMFreeDealerListRsp)
	if !t.isFree() {
...
func (t *Desk) freeBet(userid string, seatBet uint32,
	...
	t.sendCoin(userid, (-1 * num), int32(pb.LOG_TYPE127))
...
func resFreeBet(seat, beseat uint32, val, coin,
	bets int64, userid string) *pb.RMFreeBetRsp {
	return &pb.RMFreeBetRsp{
		Seat:   seat,
		Beseat: beseat,
		Value:  uint32(val),
		Coin:   coin,
...
// ' 百人输赢趋势
func (t *Desk) freeTrends() *pb.RMFreeTrendRsp {
	msg := new(pb.RMFreeTrendRsp)
...
// ' 百人上局赢家列表
func (t *Desk) freeWiners() *pb.RMFreeWinersRsp {
	msg := new(pb.RMFreeWinersRsp)
...
// ' 百人玩家列表
func (t *Desk) freeRoles() *pb.RMFreeRolesRsp {
	msg := new(pb.RMFreeRolesRsp)
...
// ' 百人坐下
func (t *Desk) freeSit(userid string, arg *pb.RMSitReq) (msg *pb.RMSitRsp) {
...
func (t *Desk) sitCheck(userid string, arg *pb.RMSitReq) pb.ErrCode {
	user := t.getPlayer(userid)
	if user == nil {
		glog.Errorf("userid %s not exist", userid)
		return pb.NotInRoom
...
func (t *Desk) shuffle() {
	...
	t.DeskGame.WildCard = d[0]                            //万能牌
	t.DeskGame.QiCards = append(t.DeskGame.QiCards, d[1]) //弃牌堆
	t.DeskGame.Cards = d[2:]
}
	...
	for k, v := range t.DeskFree.Multiple {
		...
		if val != 0 { //表示庄家赢,且大于1倍从玩家身上扣赔付倍数
			for userid, betNum := range tmp {
				...
				//扣除位置数
				t.sendCoin(userid, (-1 * int64(num)), int32(pb.LOG_TYPE128))
...
func (t *Desk) dealerJiesuan1() {
	for k, v := range t.DeskFree.Multiple {
		...
		for userid, betNum := range tmp {
			...
			//扣除位置数
			t.sendCoin(userid, val2, int32(pb.LOG_TYPE128))
...
// ' 发牌消息
func resDraw(seat uint32, state int32, cards []uint32) *pb.RMDrawNtf {
	return &pb.RMDrawNtf{
		Seat:  seat,
		State: state,
		Cards: cards,
...
func resFreeStart(dealer, photo, nickname string, state int32,
	carry int64, dealerNum, left uint32) *pb.RMFreeGamestartNtf {
	return &pb.RMFreeGamestartNtf{
		Dealer:        dealer,
		Photo:         photo,
		State:         state,
		Coin:          carry,
...
// ' 游戏结束消息
func (t *Desk) resOverFree() *pb.RMFreeGameoverNtf {
	var left uint32
	if t.DeskGame.Dealer == "" {
		left = 1
	} else {
