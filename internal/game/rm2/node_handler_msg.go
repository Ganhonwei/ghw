...
func (a *DeskActor) GetRoomList(ctx actor.Context) {
	...
	//响应
	rsp := handler.PackRMRoomList(arg, a.desks)
	arg.Sender.Tell(rsp)
}
...
func (a *DeskActor) syncConfig(arg *pb.SyncConfig, ctx actor.Context) {
	...
	if err := LoadRmRoiConfig(); err != nil {
		glog.Errorf("load rm roi config error: %v", err)
	}
}
	...
	case int32(pb.ROOM_TYPE1): //私人
		...
		if deskPid, ok := a.spawnDesk2(gameData, ctx); ok {
			...
			if arg.Gmode == 1 {
				...
				//扣除玩家消耗
				if gameData.Cost > 0 {
					msg2 := handler.ChangeCurrencyMsg((-1 * int64(gameData.Cost)), 0, 0, 0, 0, 0, int32(pb.LOG_TYPE2), arg.Cid, "RM娱乐对战房创建", "")
...
func (a *DeskActor) resetMatching() {
	...
	a.matchTimer = 0
	// glog.Infof("RM mathch reset time：%v", utils.LocalTime().Format("2006-01-02 15:04:05"))
