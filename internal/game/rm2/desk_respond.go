...
func (t *Desk) chatText(arg *pb.ChatTextReq, ctx actor.Context) {
	...
	// 	}
	// 	if vip.EmojiCost > 0 {
	// 		t.sendCurrency(userid, -int64(vip.EmojiCost), int32(pb.LOG_TYPE102), "rm双人发表情")
	// 	}
	// }
...
func (t *Desk) drawCard(userid string, area uint32, timeout bool) {
	...
	if area == 1 { //摸牌堆
		...
		seat.Touchs = append(seat.Touchs, TouchCards)

		var touched bool // 已摸牌
		// 判断进入库存控制干预流程
		if t.control != nil && t.control.controlEffect {
			if role.Robot {
				// 机器人摸牌时
				// 干预流程
				if !t.control.controlRobotDraw {
					if seat.ActTimes >= 3 {
						zlog.Infof("干预1: %s, %s, %d, %d, %d-%d, %#v", t.GameId, seat.Userid, seat.ActTimes, t.control.EffctRate, t.control.CtrlMin, t.control.CtrlMin, t.control)
						// 1.库存控制生效 或 ROI控制生效且期望玩家输
						if t.control.Ctype == 1 || (t.control.Ctype == 2 && t.control.WinRate < 0) {
							if utils.RandWan(t.control.EffctRate) {
								// 判断玩家分数 x-x 分之间
								for _, seat := range t.seats {
									if role := t.getRole(seat.Userid); role != nil && !role.Robot {
										// 玩家手牌分数
										score := algo.CalcScore(seat.SortCards, t.WildCard)
										if score >= int64(t.control.CtrlMin) && score <= int64(t.control.CtrlMax) {
											// 加到对局记录
											t.control.controlRobotDraw = true
											t.RMControl.ControlRobotDraw = true
											t.RMControl.ControlRobotDrawRound = seat.ActTimes
											zlog.Infof("干预生效: %s, %d, %d", t.GameId, seat.ActTimes, score)
											break
										} else {
											zlog.Infof("干预3: %s, %d", seat.Userid, score)
										}
									}
								}
							} else {
								zlog.Infof("干预2: %s, %d", seat.Userid, t.control.EffctRate)
							}
						}
					}
				}
				...
				// 进入干预流程，摸指定好牌
				if t.control.controlRobotDraw {
					nextCard, finish := algo.NeedNextCard(seat.Cards, t.QiCards, t.DeskGame.Cards, t.WildCard)
					zlog.Infof("robot %s 摸好牌: %s, 手牌: %s", role.Userid, algo.CardsString([]uint32{nextCard}), algo.CardsString(seat.Cards))
					if !finish {
						remain_cards, ok := algo.DealAssignCard(t.DeskGame.Cards, nextCard)
						if ok {
							t.DeskGame.Cards = remain_cards
							seat.Cards = append(seat.Cards, nextCard)
							msg.Card = nextCard
							touched = true
						} else {
							glog.Errorf("robot %s touch assign card not found: ", role.Userid)
						}
					} else {
						glog.Errorf("robot %s draw card finish: ", role.Userid)
					}
				}
			}

			// else {
			// 	zlog.Infof("player %s into control effect", role.Userid)
			// 	// 玩家摸牌，判断是第几手，可以摸到需要的牌
			// 	var canDrawNeed int
			// 	var drawedNeed = seat.DrawedNeed
			// 	var remainActs int32 // 控制剩余几手
			// 	if seat.ActTimes < 5 {
			// 		canDrawNeed = int(t.control.Hand05)
			// 		remainActs = 4 - seat.ActTimes
			// 	} else if seat.ActTimes >= 5 && seat.ActTimes < 10 {
			// 		canDrawNeed = int(t.control.Hand59)
			// 		drawedNeed -= int(t.control.Hand05)
			// 		remainActs = 9 - seat.ActTimes
			// 	} else if seat.ActTimes >= 10 && seat.ActTimes < 20 {
			// 		canDrawNeed = int(t.control.Hand1020)
			// 		drawedNeed -= int(t.control.Hand05)
			// 		drawedNeed -= int(t.control.Hand59)
			// 		remainActs = 19 - seat.ActTimes
			// 	}
			// 	if drawedNeed < 0 {
			// 		drawedNeed = 0
			// 	}
			// 	remainCanDraw := canDrawNeed - drawedNeed
			// 	if remainCanDraw > 0 && (int(remainActs) <= remainCanDraw || utils.RandWan(5000)) {
			// 		zlog.Infof("player %s draw need card, %d, %d", role.Userid, canDrawNeed, canDrawNeed)
			// 		// 摸好牌
			// 		nextCard, finish := algo.NeedNextCard(seat.Cards, t.QiCards, t.DeskGame.Cards, t.WildCard)
			// 		if !finish {
			// 			remain_cards, ok := algo.DealAssignCard(t.DeskGame.Cards, nextCard)
			...
			// 				seat.DrawedNeed++
			// 			} else {
			// 				zlog.Errorf("player %s touch assign card not found: ", role.Userid)
			// 			}
			// 		} else {
			// 			zlog.Errorf("player %s draw card finish: ", role.Userid)
			// 		}
			// 	} else {
			// 		zlog.Infof("player %s can't draw need card", role.Userid)
			// 		// 玩家摸不到需要的牌
			// 		needCards, finish := algo.NeedNextCardList(seat.Cards, t.QiCards, t.DeskGame.Cards, t.WildCard)
			// 		if !finish {
			// 			card, remain_cards, ok := algo.DealExceptCards(t.DeskGame.Cards, needCards)
			// 			if ok {
			// 				t.DeskGame.Cards = remain_cards
			...
			// 				touched = true
			// 			} else {
			// 				zlog.Warning("player %s not deal expect needCards: %s", role.Userid, algo.CardsString(needCards))
			// 			}
		...
		// 发下一张牌
		if !touched {
			card, remain_cards := algo.DealNextCard(t.DeskGame.Cards)
			t.DeskGame.Cards = remain_cards
			seat.Cards = append(seat.Cards, card)
			msg.Card = card
		}
		...
		seat.Touchs = append(seat.Touchs, TouchQiCards)
...
// 人机弃牌
func (t *Desk) isRobotDrop(seatid uint32) (drop bool) {
	// role *data.DeskRole, seat *data.DeskSeat
	userid := t.getUserid(seatid)
	role := t.getPlayer(userid)
	if role == nil {
		return
	}
	seat := t.getSeat(seatid)
	...
	// 人机判断首轮是否弃牌
	if role.Robot && t.isFirstRound(seatid) {
		...
		if player != nil && playerSeat != nil {
			// 算当前玩家弃牌率
			var playerDropRate float64
			without1stRounds := player.RmWithout1stDrop + player.RmWithout1stNotDrop
			if without1stRounds > 0 {
				playerDropRate = float64(player.RmWithout1stDrop) / float64(without1stRounds)
			}
			// 玩家手牌分数
			score := algo.CalcScore(playerSeat.SortCards, t.WildCard)
			// 人机是否有第一生命
			var rHasPureShun bool
			for _, group := range seat.SortCards {
				rHasPureShun = algo.IsPureTonghuashun(group)
				if rHasPureShun {
					break
				}
			}
			// 玩家是否有第一生命
			var pHasPureShun bool
			playerid := t.GetOnlyOnePlayer()
			if playerid != "" {
				...
				if seat != nil {
					for _, group := range seat.SortCards {
						pHasPureShun = algo.IsPureTonghuashun(group)
						if pHasPureShun {
							break
						}
					}
			...
			// 子游戏对局数
			var rmRounds int64
			if player.WinRates != nil {
				w := player.WinRates[int32(pb.RUMMY2)]
				rmRounds = w.WinRound + w.LoseRound
			}
			// 玩家收益率
			var winRate float64
			if player.Money > 0 {
				winRate = (float64(player.CashOut) + float64(player.OutDiamond)) / float64(player.Money)
			}
			zlog.Infof("人机首轮弃牌判断: %s,%s, 玩家弃牌率=%v,玩家手牌分数=%v, 人机有第一生命=%v, 子游戏对局数=%v", role.Userid, playerid, playerDropRate, score, rHasPureShun, rmRounds)
			for _, drop := range rmRoiConfig.Drop {
				// 充值次数范围
				if drop.RcCountRange != "" && !handler.AnalysisFreeStrategyParam(drop.RcCountRange, float64(len(player.RechargeTarge))) {
					continue
				}
				// 弃牌率匹配
				if drop.PDropRate != "" && !handler.AnalysisFreeStrategyParam(drop.PDropRate, playerDropRate) {
					continue
				}
				// 手牌分数匹配
				if drop.RScore != "" && !handler.AnalysisFreeStrategyParam(drop.RScore, float64(score)) {
					continue
				}
				// 人机有第一生命
				if drop.R1life != rHasPureShun {
					continue
				}
				// 玩家是否有第一生命
				if drop.P1life != pHasPureShun {
					continue
				}
				// 子游戏对局数
				if drop.SubInningsRange != "" && !handler.AnalysisFreeStrategyParam(drop.SubInningsRange, float64(rmRounds)) {
					continue
				}
				// 玩家收益率
				if drop.WinRateRange != "" && !handler.AnalysisFreeStrategyParam(drop.WinRateRange, winRate) {
					zlog.Infof("drop3 continue: %v, %v", drop.WinRateRange, winRate)
					continue
				}
				// 弃牌概率 -> 弃牌
				if utils.RandWan(int32(drop.RDropRate)) {
					zlog.Infof("人机首轮弃牌生效: %s", drop.Id)
					t.RMControl.DropId = drop.Id
					return true
				}
			}
		}
...
func (t *Desk) discard(userid string, card uint32, timeout bool) {
	...
	seat.Actions = append(seat.Actions, ActionChu)
	...
	seat.ActTimes++   //操作轮数+1

	// 记录首轮未弃牌
	t.recordPlayerDrop(seatid, userid, false)
...
func (t *Desk) finish(userid string, req *pb.RMFinishReq) {
	...
	if ret { //真胡
		seat.Actions = append(seat.Actions, ActionHu)
		...
		seat.Actions = append(seat.Actions, ActionZhaHu)
		seat.SortCards = finish
...
func (t *Desk) declare(userid string, req *pb.RMDeclareReq) {
	...
	seat.Actions = append(seat.Actions, ActionDeclare)
...
func (t *Desk) drop(userid string, req *pb.RMDropReq) {
	...
	seat.Actions = append(seat.Actions, ActionQi)
	...
	t.recordPlayerDrop(seatid, userid, true)
...
// 记录玩家首回合弃牌行为
func (t *Desk) recordPlayerDrop(seatid uint32, userid string, drop bool) {
	if !t.isFirstRound(seatid) {
		return
	}
	zlog.Infof("recordPlayerDrop %s %v, round=%d", userid, drop, t.ActTimes)
	player := t.getPlayer(userid)
	if player == nil || player.Robot {
		return
	}
	...
	var has1st bool
	for _, group := range seat.SortCards {
		if algo.IsPureTonghuashun(group) {
			has1st = true
			break
		}
	}
	...
	if drop {
		player.RmWithout1stDrop++
	} else {
		player.RmWithout1stNotDrop++
	}
}
