 ...
 * @return int64
 */
func TimestampAppiontZero(sec time.Time, loc *time.Location) int64 {
	return TimestampZeroTime(sec, loc).Unix()
}
 ...
 * @return int64
 */
func TimestampToday(loc *time.Location) int64 {
	return TimestampTodayTime(loc).Unix()
}
 ...
 * @return int64
 */
func TimestampToday2(hour, min, sec int) int64 {
	return TimestampTodayTime2(hour, min, sec).Unix()
}
 ...
 * @return time.Time
 */
func TimestampTodayTime(loc *time.Location) time.Time {
	// now := now()
	...
	// 设置 IST 时间的时分秒为零
	ist = time.Date(ist.Year(), ist.Month(), ist.Day(), 0, 0, 0, 0, loc)
	return ist
}
...
获取本地当天零点时间截
*/
func TimestampTodayTimeZone(timezone string) time.Time {
	// now := now()
	// return time.Date(now.Year(), now.Month(), now.Day(), 0, 0, 0, 0, time.Local)
 ...
 * @return time.Time
 */
func TimestampTodayTime2(hour, min, sec int) time.Time {
	now := now()
 ...
 * @return time.Time
 */
func TimestampZeroTime(t time.Time, loc *time.Location) time.Time {
	// now := now()
	// return time.Date(now.Year(), now.Month(), now.Day(), 0, 0, 0, 0, time.Local)
...
func Time2StampToMS(t time.Time) int64 {
	// 将时间转换为毫秒级时间戳
	millisecondTimestamp := t.UnixNano() / 1e6
 ...
 * @return int64
 */
func TimestampYesterday(loc *time.Location) int64 {
	return TimestampToday(loc) - 86400
}
 ...
 * @return int64
 */
func TimestampTomorrow(loc *time.Location) int64 {
	return TimestampToday(loc) + 86400
}
 ...
 * @return string
 */
func DateLocalStr() string {
	return now().Local().Format(FORMATDATA)
}

func Unix(t string) (int64, error) {
	the_time, err := time.Parse(FORMATDATA, t)
	if err == nil {
 ...
 * @return (int64, error)
 */
func Str2Local(t string, loc *time.Location) (int64, error) {
	the_time, err := time.ParseInLocation(FORMAT, t, loc)
	if err == nil {
		return the_time.Unix(), err
	}
	return 0, err
...
func Str2LocalByZone(t, timeName string) (int64, error) {
	loc, err := time.LoadLocation(timeName)
	if err != nil {
		return 0, err
	}
 ...
 * @return time.Time
 */
func Str2Time(t string, loc *time.Location) time.Time {
	time, _ := Str2Local(t, loc)
...
str格式化时间转时间戳
*/
func Str2TimeZone(t, timename string) time.Time {
	tt, _ := Str2LocalByZone(t, timename)
	loc, _ := time.LoadLocation(timename)
	return time.Unix(tt, 0).In(loc)
	// return Stamp2Time(time)
}
