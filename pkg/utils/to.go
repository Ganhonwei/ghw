...
func MustBool(s string, defaultVal ...bool) bool {
	...
	if err != nil {
		// log.Println("utils MustBool strconv.ParseBool error:", err)
		return getDefault()
	}
...
func MustInt(s string, defaultVal ...int) int {
	...
	if err != nil {
		msg := "utils MustInt strconv.Atoi error:" + err.Error()
		// 加上文件调用和行号
...
func MustInt64(s string, defaultVal ...int64) int64 {
	...
	if err != nil {
		msg := "utils MustInt64 strconv.ParseInt error:" + err.Error()
		// 加上文件调用和行号
		_, callerFile, line, ok := runtime.Caller(1)
		if ok {
			msg += fmt.Sprintf("file:%s,line:%d", callerFile, line)
		...
		// log.Println(msg)
		// log.Println("utils MustInt64 strconv.ParseInt error:", err)
		return getDefault()
	}
...
func mustFloat(s string, defaultVals ...float64) float64 {
	...
	if err != nil {
		// log.Println("utils MustFloat strconv.ParseFloat error:", err)
		return getFloatDefault()
	}
...
func ToInt64(inter interface{}, defaultVal ...int64) int64 {
	var def int64 = 0
	if len(defaultVal) > 0 {
		def = defaultVal[0]
	}
...
func ToFloat64(inter interface{}, defaultVal ...float64) (r float64) {
	var def float64 = 0
	if len(defaultVal) > 0 {
		def = defaultVal[0]
	}
...
func ToBool(inter interface{}, defaultVal ...bool) bool {
	var def bool
	if len(defaultVal) > 0 {
		def = defaultVal[0]
	}
...
func Max[T int | int64 | int32 | uint32 | float64 | float32](a, b T) T {
	if b > a {
		return b
	}
	return a
...
func Min[T int | int64 | int32 | uint32 | float64 | float32](a, b T) T {
	if b < a {
		return b
	}
	return a
...
func CopyMap[K comparable, V interface{}](dst, src map[K]V) {
	if src == nil || dst == nil {
		return
	}
	maps.Copy(dst, src)
