...
var format2 = logging.MustStringFormatter(
	`%{color}[%{level:.4s}] [%{time:2006-01-02 15:04:05.000}] [%{shortfile}] %{pid}%{color:reset} %{message}`,
)
...
func Init() {
	// var logLev string
	// flag.StringVar(&logLev, "level", "INFO", "logs at or above this threshold go to stderr")
	// flag.Parse()

	//set log level
	lev, err := logging.LogLevel(flags.Level)
	if err != nil {
		panic(err)
	}
