...
type CRASH3 struct {
	...
	RobotCrashPro int32s `xls:"人机逃离概率（万分比）"`
}

// 1.扶摇直上策略
type CRASHStrategy1 struct {
	Id         int  `xls:"策略id"`
	O          int  `xls:"策略开关（o）"`
...
func parseCrash1(f *excelize.File) (ret []CRASH1, err error) {
	sheet := "1.房间基础配置表"
	...
	if len(ret) == 0 {
		err = fmt.Errorf("CRASH配置表解析错误: %s", sheet)
		return
	}
	return
}

func parseCrash2(f *excelize.File) (ret []CRASH2, err error) {
	sheet := "2.库存配置表"
	err = decxls.UnmarshalExcelize(f, sheet, &ret)
	if err != nil {
		return
...
func parseCrash3(f *excelize.File) (ret []CRASH3, err error) {
	sheet := "3.人机策略"
	err = decxls.UnmarshalExcelize(f, sheet, &ret)
	if err != nil {
		return
...
func parseCrashStrategy1(f *excelize.File) (ret []CRASHStrategy1, err error) {
	sheet := "1.扶摇直上"
	err = decxls.UnmarshalExcelize(f, sheet, &ret)
	if err != nil {
		return
...
func parseCrashStrategy2(f *excelize.File) (ret []CRASHStrategy2, err error) {
	sheet := "2.欲薅无门"
	err = decxls.UnmarshalExcelize(f, sheet, &ret)
	if err != nil {
		return
...
func parseCrashStrategy3(f *excelize.File) (ret []CRASHStrategy3, err error) {
	sheet := "3.虚假情报"
	err = decxls.UnmarshalExcelize(f, sheet, &ret)
	if err != nil {
		return
...
func parseCrashStrategy4(f *excelize.File) (ret []CRASHStrategy4, err error) {
	sheet := "4.起死回生"
	err = decxls.UnmarshalExcelize(f, sheet, &ret)
	if err != nil {
		return
...
func parseCrashStrategy5(f *excelize.File) (ret []CRASHStrategy5, err error) {
	sheet := "5.奖池风控"
	err = decxls.UnmarshalExcelize(f, sheet, &ret)
	if err != nil {
		return
...
func parseCrashStrategy6(f *excelize.File) (ret []CRASHStrategy6, err error) {
	sheet := "6.冒险奖励"
	err = decxls.UnmarshalExcelize(f, sheet, &ret)
	if err != nil {
		return
...
func parseCrashStrategy7(f *excelize.File) (ret []CRASHStrategy7, err error) {
	sheet := "7.人狂有祸"
	err = decxls.UnmarshalExcelize(f, sheet, &ret)
	if err != nil {
		return
...
func parseCrashStrategy8(f *excelize.File) (ret []CRASHStrategy8, err error) {
	sheet := "8.高潮涌现"
	err = decxls.UnmarshalExcelize(f, sheet, &ret)
	if err != nil {
		return
...
// 更新crash配置
func updateCRASH(d []byte, save bool) error {
	...
	c1, err := parseCrash1(f)
	if err != nil {
		return err
	}
	c2, err := parseCrash2(f)
	if err != nil {
		return err
	}
	c3, err := parseCrash3(f)
	if err != nil {
		return err
	}
	...
	for _, v1 := range c2 {
		games = append(games, data.Game{
			...
			CRASH: data.CRASHGame{
				ChipLimit:        base.ChipLimit.Value,
				ChipSeat:         base.ChipSeat,
				...
				Robot: data.CRASHGameRobotStrategy{
					...
					RobotLeave: robot.RobotCrashPro.Value,
				},
			},
		})
	}
...
func updateCrashStrategy(save bool, f *excelize.File) error {
	c1, err := parseCrashStrategy1(f)
	if err != nil {
		return err
	}

	c2, err := parseCrashStrategy2(f)
	if err != nil {
		return err
	}

	c3, err := parseCrashStrategy3(f)
	if err != nil {
		return err
	}

	c4, err := parseCrashStrategy4(f)
	if err != nil {
		return err
	}

	c5, err := parseCrashStrategy5(f)
	if err != nil {
		return err
	}

	c6, err := parseCrashStrategy6(f)
	if err != nil {
		return err
	}

	c7, err := parseCrashStrategy7(f)
	if err != nil {
		return err
	}

	c8, err := parseCrashStrategy8(f)
	if err != nil {
		return err
	}

	c := data.CrashStrategy{
		Id: 1,
		FYZS: data.CRASHFYZSStrategy{
