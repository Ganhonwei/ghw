...
func BuildRegistReward(area int) []int32 {
	// bean := config.GetBeginner(1)
	bean := table.GetTables().NewbieTable.Get()
	if bean == nil {
		return []int32{5000, 5000}
	}
	list := make([]int32, 0)
	var mode1, mode2 int32 = bean.Mode1[0], bean.Mode2[0]
	if area == 1 {
		// B类用户
		mode1, mode2 = bean.Mode1[1], bean.Mode2[1]
	}
	list = append(list, mode1, mode2)
	return list
}

// 渠道包名拦截
func ChannelInterceptor(arg *pb.ADParam) bool {
	if arg == nil {
		glog.Infof("ad param is nil")
		return true
	}
	channelMap := config.GetChannelPackageMap()
	for _, c := range channelMap {
		if c.Status != 1 {
			continue
		}
		if c.Name == arg.BundleId {
			return false
		}
	}
	return true
