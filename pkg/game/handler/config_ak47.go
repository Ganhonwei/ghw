...
func (ak472 AK472S) Find(id string) AK472 {
	for _, v := range ak472 {
		if v.Id == id {
			return v
		}
...
type AK473 struct {
	Id                    int  `xls:"牌型ID"`
	PlayerWildCardWeight  ints `xls:"玩家摸万能牌的权重"`
	Player0WildCardWeight ints `xls:"玩家0张万能牌时权重"`
	Player1WildCardWeight ints `xls:"玩家1张万能牌时权重"`
	RobotWildCardWeight   ints `xls:"人机摸万能牌的权重"`
	Robot0WildCardWeight  ints `xls:"人机0张万能牌时权重"`
	Robot1WildCardWeight  ints `xls:"人机1张万能牌时权重"`
	WinRateCheck          int  `xls:"是否启用胜率检测"`
...
func parseak471(f *excelize.File) (ret []AK471, err error) {
	sheet := "1.房间基础配置表"
	...
	if len(ret) == 0 {
		err = fmt.Errorf("ak47配置表解析错误: %s", sheet)
		return
	}
	return
}

func parseak472(f *excelize.File) (ret AK472S, err error) {
	sheet := "2.房间发牌配置表"
	err = decxls.UnmarshalExcelize(f, sheet, &ret)
	if err != nil {
		return
...
func parseak473(f *excelize.File) (ret []AK473, err error) {
	sheet := "3.发牌牌型配置"
	err = decxls.UnmarshalExcelize(f, sheet, &ret)
	if err != nil {
		return
...
func parseak474(f *excelize.File) (ret []AK474, err error) {
	sheet := "4.人机策略组配置"
	err = decxls.UnmarshalExcelize(f, sheet, &ret)
	if err != nil {
		return
...
func parseak475(f *excelize.File) (ret []AK475, err error) {
	sheet := "5.人机策略配置"
	err = decxls.UnmarshalExcelize(f, sheet, &ret)
	if err != nil {
		return
...
func parseak476(f *excelize.File) (ret []AK476, err error) {
	sheet := "6.新手模式"
	err = decxls.UnmarshalExcelize(f, sheet, &ret)
	if err != nil {
		return
...
func parseak477(f *excelize.File) (ret []AK477, err error) {
	sheet := "7.控制方式"
	err = decxls.UnmarshalExcelize(f, sheet, &ret)
	if err != nil {
		return
...
// 更新ak47配置
func updateAK47(d []byte, save bool) error {
	...
	ak471, err := parseak471(f)
	if err != nil {
		return err
	}
	ak472, err := parseak472(f)
	if err != nil {
		return err
	}
	ak473, err := parseak473(f)
	if err != nil {
		return err
	}
	ak474, err := parseak474(f)
	if err != nil {
		return err
	}
	ak475, err := parseak475(f)
	if err != nil {
		return err
	}
	ak476, err := parseak476(f)
	if err != nil {
		return err
	}
	ak477, err := parseak477(f)
	if err != nil {
		return err
	}

	var v3s []data.AK47GameCardType
	for _, v3 := range ak473 {
		v3s = append(v3s, data.AK47GameCardType{
			Id:                    v3.Id,
			PlayerWildCardWeight:  v3.PlayerWildCardWeight.Value,
			Player0WildCardWeight: v3.Player0WildCardWeight.Value,
			Player1WildCardWeight: v3.Player1WildCardWeight.Value,
			RobotWildCardWeight:   v3.RobotWildCardWeight.Value,
			Robot0WildCardWeight:  v3.Robot0WildCardWeight.Value,
			Robot1WildCardWeight:  v3.Robot1WildCardWeight.Value,
	...
	var v4s []data.AK47GameRobotStrategyGroup
	for _, v4 := range ak474 {
		v4s = append(v4s, data.AK47GameRobotStrategyGroup{
			Id:                 v4.Id,
	...
	var v5s []data.AK47GameRobotStrategy
	for _, v5 := range ak475 {
		v5s = append(v5s, data.AK47GameRobotStrategy{
			Id:           v5.Id,
	...
	v6s := make(map[int]data.AK47NewbieMode)
	for _, v6 := range ak476 {
		v6s[v6.Id] = data.AK47NewbieMode{
	...
	var v7s []data.AK47ControlType
	for _, v7 := range ak477 {
		v7s = append(v7s, data.AK47ControlType{
	...
	var games []data.Game
	for _, v1 := range ak471 {
		v2 := ak472.Find(v1.Id)
		if v2.Id != v1.Id {
			return fmt.Errorf("ak472 find error")
		}
